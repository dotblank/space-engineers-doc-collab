          ...
          while (true)
          {
            ...
            if (reader.Token != JsonToken.ObjectEnd)
            {
              ...
              if (objectMetadata.Properties.ContainsKey(key))
              {
                ...
                {
                  ((FieldInfo) propertyMetadata.Info).SetValue(obj1, JsonMapper.ReadValue(propertyMetadata.Type, reader));
                }
                else
                {
                  PropertyInfo propertyInfo = (PropertyInfo) propertyMetadata.Info;
                  if (propertyInfo.CanWrite)
                    propertyInfo.SetValue(obj1, JsonMapper.ReadValue(propertyMetadata.Type, reader), (object[]) null);
                  else
        ...
        else
        {
          ...
          foreach (PropertyMetadata propertyMetadata in (IEnumerable<PropertyMetadata>) list)
          {
            ...
            else
            {
              ...
              {
                writer.WritePropertyName(propertyMetadata.Info.Name);
                JsonMapper.WriteValue(propertyInfo.GetValue(obj, (object[]) null), writer, writer_is_private, depth + 1);
              }
            }
          }
          writer.WriteObjectEnd();
