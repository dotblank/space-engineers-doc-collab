...
namespace LitJson
{
    public class JsonMapper
    {
        ...
        private static IDictionary<Type, ArrayMetadata> array_metadata =
            (IDictionary<Type, ArrayMetadata>) new Dictionary<Type, ArrayMetadata>();

        private static IDictionary<Type, IDictionary<Type, MethodInfo>> conv_ops =
            (IDictionary<Type, IDictionary<Type, MethodInfo>>) new Dictionary<Type, IDictionary<Type, MethodInfo>>();

        private static IDictionary<Type, ObjectMetadata> object_metadata =
            (IDictionary<Type, ObjectMetadata>) new Dictionary<Type, ObjectMetadata>();

        private static IDictionary<Type, IList<PropertyMetadata>> type_properties =
            (IDictionary<Type, IList<PropertyMetadata>>) new Dictionary<Type, IList<PropertyMetadata>>();
        ...
        private static IDictionary<Type, ExporterFunc> base_exporters_table =
            (IDictionary<Type, ExporterFunc>) new Dictionary<Type, ExporterFunc>();

        private static IDictionary<Type, ExporterFunc> custom_exporters_table =
            (IDictionary<Type, ExporterFunc>) new Dictionary<Type, ExporterFunc>();

        private static IDictionary<Type, IDictionary<Type, ImporterFunc>> base_importers_table =
            (IDictionary<Type, IDictionary<Type, ImporterFunc>>) new Dictionary<Type, IDictionary<Type, ImporterFunc>>();
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            objectMetadata.Properties =
                (IDictionary<string, PropertyMetadata>) new Dictionary<string, PropertyMetadata>();
            foreach (PropertyInfo propertyInfo in type.GetProperties())
            {
                if (propertyInfo.Name == "Item")
                {
        ...
        private static void AddTypeProperties(Type type)
        {
            if (JsonMapper.type_properties.ContainsKey(type))
                return;
            IList<PropertyMetadata> list = (IList<PropertyMetadata>) new List<PropertyMetadata>();
            foreach (PropertyInfo propertyInfo in type.GetProperties())
            {
                if (!(propertyInfo.Name == "Item"))
        ...
        private static object ReadValue(Type inst_type, JsonReader reader)
        {
            ...
            else if (reader.Token == JsonToken.Double || reader.Token == JsonToken.Int ||
                     (reader.Token == JsonToken.Long || reader.Token == JsonToken.String) ||
                     reader.Token == JsonToken.Boolean)
            {
                Type type = reader.Value.GetType();
                if (inst_type.IsAssignableFrom(type))
                    return reader.Value;
                if (JsonMapper.custom_importers_table.ContainsKey(type) &&
                    JsonMapper.custom_importers_table[type].ContainsKey(inst_type))
                    return JsonMapper.custom_importers_table[type][inst_type](reader.Value);
                if (JsonMapper.base_importers_table.ContainsKey(type) &&
                    JsonMapper.base_importers_table[type].ContainsKey(inst_type))
                ...
                if (!(convOp != (MethodInfo) null))
                    throw new JsonException(string.Format("Can't assign value '{0}' (type {1}) to type {2}",
                        reader.Value, (object) type, (object) inst_type));
                    ...
                    while (true)
                    {
                        ...
                        if (reader.Token != JsonToken.ObjectEnd)
                        {
                            ...
                            if (objectMetadata.Properties.ContainsKey(key))
                            {
                                ...
                                {
                                    ((FieldInfo) propertyMetadata.Info).SetValue(obj1,
                                ...
                                else
                                {
                                    ...
                                    if (propertyInfo.CanWrite)
                                        propertyInfo.SetValue(obj1,
                                            JsonMapper.ReadValue(propertyMetadata.Type, reader), (object[]) null);
                                    else
                                        JsonMapper.ReadValue(propertyMetadata.Type, reader);
                                }
                            }
                            else if (!objectMetadata.IsDictionary)
                            {
                                ...
                                ((IDictionary) obj1).Add((object) key,
                                    JsonMapper.ReadValue(objectMetadata.ElementType, reader));
                        }
                        else
                            goto label_45;
                    }
                    throw new JsonException(string.Format("The type {0} doesn't have the property '{1}'",
                        (object) inst_type, (object) key));
                }
                label_45:
                return obj1;
            }
        ...
        {
            JsonMapper.base_exporters_table[typeof (byte)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToInt32((byte) obj)));
            JsonMapper.base_exporters_table[typeof (char)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToString((char) obj)));
            JsonMapper.base_exporters_table[typeof (DateTime)] =
                (ExporterFunc)
                    ((obj, writer) => writer.Write(Convert.ToString((DateTime) obj, JsonMapper.datetime_format)));
            JsonMapper.base_exporters_table[typeof (Decimal)] =
                (ExporterFunc) ((obj, writer) => writer.Write((Decimal) obj));
            JsonMapper.base_exporters_table[typeof (sbyte)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToInt32((sbyte) obj)));
            JsonMapper.base_exporters_table[typeof (short)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToInt32((short) obj)));
            JsonMapper.base_exporters_table[typeof (ushort)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToInt32((ushort) obj)));
            JsonMapper.base_exporters_table[typeof (uint)] =
                (ExporterFunc) ((obj, writer) => writer.Write(Convert.ToUInt64((uint) obj)));
            JsonMapper.base_exporters_table[typeof (ulong)] =
                (ExporterFunc) ((obj, writer) => writer.Write((ulong) obj));
        }
        ...
        private static void RegisterBaseImporters()
        {
            ...
            ImporterFunc importer12 =
                (ImporterFunc) (input => (object) Convert.ToDateTime((string) input, JsonMapper.datetime_format));
        ...
        private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
        {
            ...
            else
            {
                ...
                else if (obj is Enum)
                {
                    ...
                    if (underlyingType == typeof (long) || underlyingType == typeof (uint) ||
                ...
                else
                {
                    ...
                    writer.WriteObjectStart();
                    foreach (PropertyMetadata propertyMetadata in (IEnumerable<PropertyMetadata>) list)
                    {
                        if (propertyMetadata.IsField)
                        {
                            writer.WritePropertyName(propertyMetadata.Info.Name);
                            JsonMapper.WriteValue(((FieldInfo) propertyMetadata.Info).GetValue(obj), writer,
                                writer_is_private, depth + 1);
                        }
                        else
                        {
                            PropertyInfo propertyInfo = (PropertyInfo) propertyMetadata.Info;
                            ...
                            {
                                writer.WritePropertyName(propertyMetadata.Info.Name);
                                JsonMapper.WriteValue(propertyInfo.GetValue(obj, (object[]) null), writer,
                                    writer_is_private, depth + 1);
                            }
                        }
                    }
        ...
        public static JsonData ToObject(TextReader reader)
        {
            return
                (JsonData)
                    JsonMapper.ToWrapper((WrapperFactory) (() => (IJsonWrapper) new JsonData()), new JsonReader(reader));
        }
