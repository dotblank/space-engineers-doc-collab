...
namespace LitJson
{
    internal class Lexer
    {
        ...
        private static bool State3(FsmContext ctx)
        {
            while (ctx.L.GetChar())
            {
                if (ctx.L.input_char >= 48 && ctx.L.input_char <= 57)
                    ctx.L.string_buffer.Append((char) ctx.L.input_char);
                else if (ctx.L.input_char == 32 || ctx.L.input_char >= 9 && ctx.L.input_char <= 13)
                {
                    ctx.Return = true;
                    ctx.NextState = 1;
                    return true;
        ...
        private static bool State22(FsmContext ctx)
        {
            ...
            while (ctx.L.GetChar())
            {
                if ((ctx.L.input_char < 48 || ctx.L.input_char > 57) && (ctx.L.input_char < 65 || ctx.L.input_char > 70) &&
                    (ctx.L.input_char < 97 || ctx.L.input_char > 102))
                    return false;
        ...
        public bool NextToken()
        {
            ...
            for (; Lexer.fsm_handler_table[this.state - 1](this.fsm_context); this.state = this.fsm_context.NextState)
            {
                ...
                {
                    this.string_value = ((object) this.string_buffer).ToString();
