...
namespace System.Linq.Expressions
{
  public static class ExpressionExtension
  {
    ...
    public static TDelegate InstanceCall<TDelegate>(this MethodInfo info)
    {
      ...
      ParameterExpression[] parameterExpressionArray = Enumerable.ToArray<ParameterExpression>(Enumerable.Select<ParameterInfo, ParameterExpression>((IEnumerable<ParameterInfo>) parameters1, (Func<ParameterInfo, ParameterExpression>) (s => Expression.Parameter(s.ParameterType, s.Name))));
      Expression[] expressionArray = new Expression[parameters2.Length];
      for (int index = 0; index < parameters2.Length; ++index)
        expressionArray[index] = !(parameters2[index].ParameterType == parameters1[index].ParameterType) ? (Expression) Expression.Convert((Expression) parameterExpressionArray[index + 1], parameters2[index].ParameterType) : (Expression) parameterExpressionArray[index + 1];
      return Expression.Lambda<TDelegate>((Expression) Expression.Call(parameters1[0].ParameterType == info.DeclaringType ? (Expression) parameterExpressionArray[0] : (Expression) Expression.Convert((Expression) parameterExpressionArray[0], info.DeclaringType), info, expressionArray), parameterExpressionArray).Compile();
    }
  }
}
