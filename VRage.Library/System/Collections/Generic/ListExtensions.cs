...
namespace System.Collections.Generic
{
    public static class ListExtensions
    {
        ...
        private static class ListInternalAccessor<T>
        {
            ...
            {
                DynamicMethod dynamicMethod1 = new DynamicMethod("get",
                ...
                ilGenerator1.Emit(OpCodes.Ldfld,
                    typeof (List<T>).GetField("_items", BindingFlags.Instance | BindingFlags.NonPublic));
                ilGenerator1.Emit(OpCodes.Ret);
                ListExtensions.ListInternalAccessor<T>.GetArray =
                    (Func<List<T>, T[]>) dynamicMethod1.CreateDelegate(typeof (Func<List<T>, T[]>));
                DynamicMethod dynamicMethod2 = new DynamicMethod("set",
                    MethodAttributes.Public | MethodAttributes.Static, CallingConventions.Standard, (Type) null,
                    new Type[2]
                    {
                        typeof (List<T>),
                        typeof (int)
                ...
                ilGenerator2.Emit(OpCodes.Stfld,
                    typeof (List<T>).GetField("_size", BindingFlags.Instance | BindingFlags.NonPublic));
                ...
                ListExtensions.ListInternalAccessor<T>.SetSize =
                    (Action<List<T>, int>) dynamicMethod2.CreateDelegate(typeof (Action<List<T>, int>));
            }
        }
    }
}