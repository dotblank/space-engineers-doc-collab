    ...
    public class Hashtable<Key, Data> : IEnumerable<KeyValuePair<Key, Data>>, IEnumerable
    {
        ...
        private bool Insert(Hashtable<Key, Data>.Node[] table, Key key, Data data)
        {
            ...
            do
            {
                ...
                if (node.Token == Hashtable<Key, Data>.Token.Empty || node.Token == Hashtable<Key, Data>.Token.Deleted ||
                    Hashtable<Key, Data>.KeyComparer.Equals(key, node.Key))
                {
        ...
        private bool Find(Key key, out Hashtable<Key, Data>.Node node)
        {
            ...
            do
            {
                ...
                if (node1.Token == Hashtable<Key, Data>.Token.Empty)
                    return false;
                if (node1.Token == Hashtable<Key, Data>.Token.Deleted ||
                    !Hashtable<Key, Data>.KeyComparer.Equals(key, node1.Key))
                    index = (index + 1)%nodeArray.Length;
                else
                    goto label_5;
            } while (index != num);
        ...
        public void Remove(Key key)
        {
            try
            {
                ...
                do
                {
                    ...
                    {
                        if (node.Token == Hashtable<Key, Data>.Token.Deleted ||
                            !Hashtable<Key, Data>.KeyComparer.Equals(key, node.Key))
                            index = (index + 1)%nodeArray.Length;
                        else
