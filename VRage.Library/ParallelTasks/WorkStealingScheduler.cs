    ...
    public class WorkStealingScheduler : IWorkScheduler
    {
        ...
        public bool WaitForTasksToFinish(TimeSpan waitTimeout)
        {
            return
                Parallel.WaitForAll(
                    (WaitHandle[])
                        Enumerable.ToArray<ManualResetEvent>(
                            Enumerable.Select<Worker, ManualResetEvent>((IEnumerable<Worker>) this.Workers,
                                (Func<Worker, ManualResetEvent>) (s => s.HasNoWork))), waitTimeout);
        }
    }
}