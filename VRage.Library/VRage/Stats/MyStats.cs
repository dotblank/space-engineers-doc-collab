...
namespace VRage.Stats
{
    ...
    {
        private static Comparer<KeyValuePair<string, MyStat>> m_nameComparer =
            (Comparer<KeyValuePair<string, MyStat>>) new MyNameComparer();
        ...
        public MyStatToken Measure(string name, MyStatTypeEnum type = MyStatTypeEnum.Avg, int refreshMs = 200,
            int numDecimals = 1, int clearRateMs = -1)
        {
            MyStat stat = this.GetStat(name);
            if (stat.DrawText == null)
                stat.DrawText = this.GetMeasureText(name, type);
            stat.ChangeSettings((type | MyStatTypeEnum.FormatFlag) & (MyStatTypeEnum) 191, refreshMs, numDecimals,
                clearRateMs);
        ...
        public void Write(string name, float value, MyStatTypeEnum type, int refreshMs, int numDecimals,
            int clearRateMs = -1)
        {
            this.GetStat(name).Write(value, type, refreshMs, numDecimals, clearRateMs);
        }

        public void Write(string name, long value, MyStatTypeEnum type, int refreshMs, int numDecimals,
            int clearRateMs = -1)
        {
            this.GetStat(name).Write(value, type, refreshMs, numDecimals, clearRateMs);
        }

        public void WriteFormat(string name, float value, MyStatTypeEnum type, int refreshMs, int numDecimals,
            int clearRateMs = -1)
        {
            this.GetStat(name).Write(value, type | MyStatTypeEnum.FormatFlag, refreshMs, numDecimals, clearRateMs);
        }

        public void WriteFormat(string name, long value, MyStatTypeEnum type, int refreshMs, int numDecimals,
            int clearRateMs = -1)
        {
            this.GetStat(name).Write(value, type | MyStatTypeEnum.FormatFlag, refreshMs, numDecimals, clearRateMs);
        }
        ...
        private void AppendStatLine<A, B, C>(StringBuilder text, string statName, A arg0, B arg1, C arg2,
            NumberFormatInfo format, string formatString) where A : IConvertible where B : IConvertible
            where C : IConvertible
        {
            if (formatString == null)
                StringBuilderExtensions.ConcatFormat<A, B, C>(text, statName, arg0, arg1, arg2, format);
            else
                StringBuilderExtensions.ConcatFormat<string, A, B, C>(text, formatString, statName, arg0, arg1, arg2,
                    format);
            text.AppendLine();
        }
        ...
        private void AppendStat(StringBuilder text, string statKey, MyStat stat)
        {
            ...
            stat.ReadAndClear(this.m_timer.Elapsed, out sum, out count, out min, out max, out last, out type,
                out decimals, out inactivityMs);
            ...
            else
            {
                ...
                switch (type & (MyStatTypeEnum) 15)
                {
                    case MyStatTypeEnum.CurrentValue:
                        if (flag1)
                        {
                            this.AppendStatLine<long, int, int>(text, statName, last.AsLong, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, int, int>(text, statName, last.AsFloat, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                    case MyStatTypeEnum.Min:
                        if (flag1)
                        {
                            this.AppendStatLine<long, int, int>(text, statName, min.AsLong, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, int, int>(text, statName, min.AsFloat, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                    case MyStatTypeEnum.Max:
                        if (flag1)
                        {
                            this.AppendStatLine<long, int, int>(text, statName, max.AsLong, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, int, int>(text, statName, max.AsFloat, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                    case MyStatTypeEnum.Avg:
                        this.AppendStatLine<float, int, int>(text, statName, num, 0, 0, this.m_format,
                            flag2 ? (string) null : "{0}: {1}");
                        break;
                    case MyStatTypeEnum.MinMax:
                        if (flag1)
                        {
                            this.AppendStatLine<long, long, int>(text, statName, min.AsLong, max.AsLong, 0,
                                this.m_format, flag2 ? (string) null : "{0}: {1} / {2}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, float, int>(text, statName, min.AsFloat, max.AsFloat, 0,
                                this.m_format, flag2 ? (string) null : "{0}: {1} / {2}");
                            break;
                        }
                    case MyStatTypeEnum.MinMaxAvg:
                        if (flag1)
                        {
                            this.AppendStatLine<long, long, float>(text, statName, min.AsLong, max.AsLong, num,
                                this.m_format, flag2 ? (string) null : "{0}: {1} / {2} / {3}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, float, float>(text, statName, min.AsFloat, max.AsFloat, num,
                                this.m_format, flag2 ? (string) null : "{0}: {1} / {2} / {3}");
                            break;
                        }
                    case MyStatTypeEnum.Sum:
                        if (flag1)
                        {
                            this.AppendStatLine<long, int, int>(text, statName, sum.AsLong, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<float, int, int>(text, statName, sum.AsFloat, 0, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1}");
                            break;
                        }
                    case MyStatTypeEnum.Counter:
                        this.AppendStatLine<int, int, int>(text, statName, count, 0, 0, this.m_format,
                            flag2 ? (string) null : "{0}: {1}");
                        break;
                    case MyStatTypeEnum.CounterSum:
                        if (flag1)
                        {
                            this.AppendStatLine<int, long, int>(text, statName, count, sum.AsLong, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1} / {2}");
                            break;
                        }
                        else
                        {
                            this.AppendStatLine<int, float, int>(text, statName, count, sum.AsFloat, 0, this.m_format,
                                flag2 ? (string) null : "{0}: {1} / {2}");
                            break;
                        }
                }
