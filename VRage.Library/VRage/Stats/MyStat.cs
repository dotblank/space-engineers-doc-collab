...
namespace VRage.Stats
{
    internal class MyStat
    {
        ...
        public void ReadAndClear(MyTimeSpan currentTime, out MyStat.Value sum, out int count, out MyStat.Value min,
            out MyStat.Value max, out MyStat.Value last, out MyStatTypeEnum type, out int decimals,
            out MyTimeSpan inactivityMs)
        {
            ...
            try
            {
                ...
                if (this.Count <= 0)
                {
                    MyTimeSpan delta = this.IntToDeltaTime(-this.Count) +
                                       (this.Count < 0 ? currentTime - this.LastClear : MyTimeSpan.FromMiliseconds(1.0));
                ...
                else
                {
                    ...
                    if (this.ClearRate != -1 &&
                        currentTime >= this.LastClear + MyTimeSpan.FromMiliseconds((double) this.ClearRate))
                    {
                        this.Count = 0;
                        this.ClearUnsafe();
                        this.LastClear = currentTime;
