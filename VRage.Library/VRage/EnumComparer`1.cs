  ...
  public sealed class EnumComparer<TEnum> : IEqualityComparer<TEnum> where TEnum : struct, IComparable, IConvertible, IFormattable
  {
    ...
    private static void AssertTypeIsEnum()
    {
      if (!typeof (TEnum).IsEnum)
        throw new NotSupportedException(string.Format("The type parameter {0} is not an Enum. LcgEnumComparer supports Enums only.", (object) typeof (TEnum)));
    }
    ...
    private static void AssertUnderlyingTypeIsSupported()
    {
      ...
      if (!((ICollection<Type>) new Type[8]
      {
        ...
        throw new NotSupportedException(string.Format("The underlying type of the type parameter {0} is {1}. LcgEnumComparer only supports Enums with underlying type of byte, sbyte, short, ushort, int, uint, long, or ulong.", (object) typeof (TEnum), (object) underlyingType));
    }
