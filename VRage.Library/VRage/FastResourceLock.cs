...
namespace VRage
{
    public sealed class FastResourceLock : IDisposable, IResourceLock
    {
        ...
        {
            get { return this._value >> 22 & 1023; }
        }

        public bool Owned
        {
            get { return (this._value & 1) != 0; }
        }

        public int SharedOwners
        {
            get { return this._value >> 2 & 1023; }
        }

        public int SharedWaiters
        {
            get { return this._value >> 12 & 1023; }
        }
        ...
        public void ReleaseShared()
        {
            ...
            do
            {
                ...
                {
                    if (Interlocked.CompareExchange(ref this._value, comparand - 1 + 2 - 4 - 4194304, comparand) ==
                        comparand)
                    {
                        NativeMethods.ReleaseSemaphore(this._exclusiveWakeEvent, 1, IntPtr.Zero);
                        break;
        ...
        public void SpinAcquireExclusive()
        {
            while (true)
            {
                int comparand = this._value;
                if ((comparand & 3) != 0 ||
                    Interlocked.CompareExchange(ref this._value, comparand + 1, comparand) != comparand)
                {
        ...
        public void SpinAcquireShared()
        {
            while (true)
            {
                ...
                if ((comparand & -4194302) == 0)
                {
                    ...
                    else if ((comparand >> 2 & 1023) != 0 &&
                             Interlocked.CompareExchange(ref this._value, comparand + 4, comparand) == comparand)
                        goto label_3;
                }
                if (NativeMethods.SpinEnabled)
                    Thread.SpinWait(8);
