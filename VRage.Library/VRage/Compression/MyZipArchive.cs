...
namespace VRage.Compression
{
  ...
  {
    private Dictionary<string, string> m_mixedCaseHelper = new Dictionary<string, string>((IEqualityComparer<string>) StringComparer.InvariantCultureIgnoreCase);
    ...
    public IEnumerable<string> FileNames
    {
      get
      {
        return (IEnumerable<string>) Enumerable.OrderBy<string, string>(Enumerable.Select<MyZipFileInfo, string>((IEnumerable<MyZipFileInfo>) this.Files, (Func<MyZipFileInfo, string>) (p => p.Name)), (Func<string, string>) (p => p));
      }
    }
    ...
    public MyZipArchive.Enumerator Files
    {
      get
      {
        return new MyZipArchive.Enumerator(((IEnumerable) MyZipArchiveReflection.GetFiles(this.m_zip)).GetEnumerator());
      }
    }
    ...
    {
      return new MyZipFileInfo(MyZipArchiveReflection.AddFile(this.m_zip, path, (ushort) compressionMethod, (byte) deflateOption));
    }
    ...
    public static bool IsHidden(FileInfo f, DirectoryInfo relativeTo)
    {
      if ((f.Attributes & FileAttributes.Hidden) != (FileAttributes) 0)
        return true;
      for (DirectoryInfo directoryInfo = f.Directory; !directoryInfo.FullName.Equals(relativeTo.FullName, StringComparison.InvariantCultureIgnoreCase); directoryInfo = directoryInfo.Parent)
      {
    ...
    public static void CreateFromDirectory(string sourceDirectoryName, string destinationArchiveFileName, DeflateOptionEnum compressionLevel, bool includeBaseDirectory, string[] ignoredExtensions = null, bool includeHidden = true)
    {
      ...
      using (MyZipArchive myZipArchive = MyZipArchive.OpenOnFile(destinationArchiveFileName, FileMode.Create, FileAccess.ReadWrite, FileShare.None, false))
      {
        ...
        foreach (FileInfo f in relativeTo.GetFiles("*", SearchOption.AllDirectories))
        {
          if (includeHidden || !MyZipArchive.IsHidden(f, relativeTo))
          {
            ...
            if (ignoredExtensions == null || !Enumerable.Contains<string>((IEnumerable<string>) ignoredExtensions, Path.GetExtension(fullName), (IEqualityComparer<string>) StringComparer.InvariantCultureIgnoreCase))
            {
              using (FileStream fileStream = File.Open(fullName, FileMode.Open, FileAccess.Read, FileShare.Read))
              {
                using (Stream stream = myZipArchive.AddFile(fullName.Substring(startIndex), CompressionMethodEnum.Deflated, compressionLevel).GetStream(FileMode.Open, FileAccess.Write))
    ...
    public static void ExtractToDirectory(string sourceArchiveFileName, string destinationDirectoryName)
    {
      ...
      using (MyZipArchive myZipArchive = MyZipArchive.OpenOnFile(sourceArchiveFileName, FileMode.Open, FileAccess.Read, FileShare.Read, false))
      {
    ...
    public struct Enumerator : IEnumerator<MyZipFileInfo>, IDisposable, IEnumerator, IEnumerable<MyZipFileInfo>, IEnumerable
    {
      ...
      public MyZipFileInfo Current
      {
        get
        {
          return new MyZipFileInfo(this.m_enumerator.Current);
        }
      }
