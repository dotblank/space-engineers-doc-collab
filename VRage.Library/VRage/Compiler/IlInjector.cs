...
namespace VRage.Compiler
{
  public class IlInjector
  {
    ...
    public static Assembly InjectCodeToAssembly(string newAssemblyName, Assembly inputAssembly, MethodInfo method)
    {
      ...
      AssemblyBuilder assemblyBuilder = Thread.GetDomain().DefineDynamicAssembly(name, flag ? AssemblyBuilderAccess.RunAndSave : AssemblyBuilderAccess.Run);
    ...
    private static void InjectTypes(Type[] sourceTypes, ModuleBuilder newModule, MethodInfo methodToInject)
    {
      ...
      Dictionary<ConstructorBuilder, ConstructorInfo> dictionary3 = new Dictionary<ConstructorBuilder, ConstructorInfo>();
      ...
      foreach (KeyValuePair<TypeBuilder, Type> keyValuePair1 in dictionary1)
      {
        foreach (KeyValuePair<MethodBuilder, MethodInfo> keyValuePair2 in dictionary2)
        {
          if (keyValuePair2.Key.DeclaringType == (Type) keyValuePair1.Key)
            IlInjector.InjectMethod((MethodBase) keyValuePair2.Value, keyValuePair2.Key.GetILGenerator(), list, dictionary2, dictionary3, dictionary1, methodToInject);
        }
    ...
    private static TypeBuilder CreateType(ModuleBuilder newModule, Dictionary<TypeBuilder, Type> createdTypes, Type sourceType)
    {
      ...
      TypeBuilder key = newModule.DefineType(sourceType.Name, attr, sourceType.BaseType, sourceType.GetInterfaces());
    ...
    {
      foreach (FieldInfo fieldInfo in sourceType.GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.SetField))
    ...
    {
      foreach (PropertyInfo propertyInfo in sourceType.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.GetProperty | BindingFlags.SetProperty))
        newType.DefineProperty(propertyInfo.Name, PropertyAttributes.HasDefault, propertyInfo.PropertyType, Type.EmptyTypes);
    }
    ...
    private static void CopyMethods(Dictionary<MethodBuilder, MethodInfo> createdMethods, Type type, TypeBuilder newType)
    {
      foreach (MethodInfo methodInfo in type.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
      {
        if (!(methodInfo.DeclaringType != type))
        {
          ...
          createdMethods.Add(newType.DefineMethod(methodInfo.Name, methodInfo.Attributes, methodInfo.CallingConvention, methodInfo.ReturnType, parameterTypes), methodInfo);
        }
      }
    }
    ...
    private static void CopyConstructors(Dictionary<ConstructorBuilder, ConstructorInfo> createdConstructors, Type type, TypeBuilder newType)
    {
      foreach (ConstructorInfo constructorInfo in type.GetConstructors(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
      {
        if (!(constructorInfo.DeclaringType != type))
        {
          ...
          foreach (ParameterInfo parameterInfo in parameters)
            parameterTypes[num++] = parameterInfo.ParameterType;
          createdConstructors.Add(newType.DefineConstructor(constructorInfo.Attributes, constructorInfo.CallingConvention, parameterTypes), constructorInfo);
        }
      }
    }
    ...
    {
      IlInjector.ConstructInstructions(sourceMethod, methodGenerator, fields, methods, constructors, types, methodToInject);
    }
        ...
        else
        {
          switch (instruction.OpCode.OperandType)
          {
            ...
            case OperandType.InlineField:
              IlInjector.ResolveField(instruction.Operand as FieldInfo, createdFields, methodGenerator, opCode);
              continue;
            ...
            case OperandType.InlineMethod:
              try
              {
                IlInjector.ResolveMethod(methodGenerator, createdMethods, createdConstructors, instruction, opCode);
                continue;
              }
              catch
              {
    ...
    private static void ResolveMethod(ILGenerator generator, Dictionary<MethodBuilder, MethodInfo> methods, Dictionary<ConstructorBuilder, ConstructorInfo> constructors, IlReader.IlInstruction instruction, OpCode code)
    {
      bool flag = false;
