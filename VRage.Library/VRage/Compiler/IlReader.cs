        ...
        switch (this.OpCode.OperandType)
        {
          ...
          case OperandType.InlineType:
            if (this.Operand is MethodInfo)
            {
              ...
              string str = methodInfo.ToString().Substring(((object) methodInfo.ReturnType.Name).ToString().Length + 1);
              return string.Format("{0} {1}::{2}", (object) methodInfo.ReturnType, (object) methodInfo.DeclaringType, (object) str);
            }
            else
            {
