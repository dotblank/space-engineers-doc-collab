...
namespace VRage.Compiler
{
  public class IlCompiler
  {
        ...
        foreach (Type classType in assembly.GetTypes())
        {
          ...
          foreach (MethodBase method in list)
          {
            if (IlChecker.IsMethodFromParent(classType, method))
            {
              ...
              {
                errors.Add(string.Format("Class {0} derives from class {1} that is not allowed in script", (object) classType.Name, (object) method.DeclaringType.Name));
                return false;
              }
            }
            else
    ...
    public static bool Compile(string[] instructions, out Assembly assembly, bool isIngameScript, bool wrap = true)
    {
      ...
      if (wrap)
        IlCompiler.m_cache.AppendFormat("public static class wrapclass{{ public static object run() {{ {0} return null;}} }}", (object[]) instructions);
      else
        IlCompiler.m_cache.Append(instructions[0]);
      CompilerResults compilerResults = IlCompiler.m_cp.CompileAssemblyFromSource(IlCompiler.Options, ((object) IlCompiler.m_cache).ToString());
      ...
      foreach (Type type in assembly.GetTypes())
      {
        ...
        {
          Type failed;
          if (!(type == typeof (MulticastDelegate)) && !IlChecker.CheckIl(IlCompiler.m_reader.ReadInstructions((MethodBase) methodInfo), out failed, isIngameScript, allowedTypes))
          {
            assembly = (Assembly) null;
            return false;
          }
