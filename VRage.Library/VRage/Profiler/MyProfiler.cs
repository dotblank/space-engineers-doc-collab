...
namespace VRage.Profiler
{
  public class MyProfiler
  {
    ...
    private Dictionary<MyProfiler.MyProfilerBlockKey, MyProfiler.MyProfilerBlock> m_profilingBlocks = new Dictionary<MyProfiler.MyProfilerBlockKey, MyProfiler.MyProfilerBlock>(8192, (IEqualityComparer<MyProfiler.MyProfilerBlockKey>) new MyProfiler.MyProfilerBlockKeyComparer());
    ...
    private Dictionary<MyProfiler.MyProfilerBlockKey, MyProfiler.MyProfilerBlock> m_blocksToAdd = new Dictionary<MyProfiler.MyProfilerBlockKey, MyProfiler.MyProfilerBlock>(8192, (IEqualityComparer<MyProfiler.MyProfilerBlockKey>) new MyProfiler.MyProfilerBlockKeyComparer());
    ...
    private void CommitInternal()
    {
      if (this.m_blocksToAdd.Count > 0)
      {
        ...
        {
          foreach (KeyValuePair<MyProfiler.MyProfilerBlockKey, MyProfiler.MyProfilerBlock> keyValuePair in this.m_blocksToAdd)
          {
      ...
      foreach (MyProfiler.MyProfilerBlock myProfilerBlock in this.m_profilingBlocks.Values)
      {
        ...
        myProfilerBlock.averageMiliseconds = (float) (0.899999976158142 * (double) myProfilerBlock.averageMiliseconds + 0.100000001490116 * myProfilerBlock.Elapsed.Miliseconds);
    ...
    public void StartBlock(string name, string memberName, int line, string file)
    {
      ...
      else
      {
        ...
        MyProfiler.MyProfilerBlockKey key = new MyProfiler.MyProfilerBlockKey(file, memberName, name, line, this.GetParentId());
    ...
    {
      if ((!MyProfiler.EnableAsserts || profilingBlock.Key.Member.Equals(member)) && (profilingBlock.Key.ParentId == parentId && !(profilingBlock.Key.File != file)))
        return;
    ...
    public void EndBlock(string member, int line, string file, MyTimeSpan? customTime = null, float customValue = 0.0f, string timeFormat = null, string valueFormat = null)
    {
    ...
    public class MyProfilerBlock
    {
      ...
      public float ManagedDeltaMB
      {
        get
        {
          return this.DeltaManagedB * 9.53674E-07f;
        }
      }
      ...
      public float ProcessDeltaMB
      {
        get
        {
          return this.DeltaProcessB * 9.53674E-07f;
        }
      }
