  ...
  internal sealed class TupleSerializer : IProtoTypeSerializer, IProtoSerializer
  {
    ...
    public TupleSerializer(RuntimeTypeModel model, ConstructorInfo ctor, MemberInfo[] members)
    {
      ...
      for (int index = 0; index < members.Length; ++index)
      {
        ...
        IProtoSerializer coreSerializer = ValueMember.TryGetCoreSerializer(model, DataFormat.Default, type, out defaultWireType, asReference, false, false, true);
        if (coreSerializer == null)
          throw new InvalidOperationException("No serializer defined for type: " + type.FullName);
        IProtoSerializer tail = (IProtoSerializer) new TagDecorator(index + 1, defaultWireType, false, coreSerializer);
        IProtoSerializer protoSerializer = !(itemType == (Type) null) ? (!parameterType.IsArray ? (IProtoSerializer) new ListDecorator((TypeModel) model, parameterType, defaultType, tail, index + 1, false, defaultWireType, true, false, false) : (IProtoSerializer) new ArrayDecorator((TypeModel) model, tail, index + 1, false, defaultWireType, parameterType, false, false)) : tail;
    ...
    public object Read(object value, ProtoReader source)
    {
      ...
      while ((num = source.ReadFieldHeader()) > 0)
      {
        ...
        if (num <= this.tails.Length)
        {
          ...
          parameters[num - 1] = this.tails[num - 1].Read(protoSerializer.RequiresOldValue ? parameters[num - 1] : (object) null, source);
        }
        else
          source.SkipField();
      }
