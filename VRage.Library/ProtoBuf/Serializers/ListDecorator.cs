...
namespace ProtoBuf.Serializers
{
  internal sealed class ListDecorator : ProtoDecoratorBase
  {
    ...
    protected override void EmitRead(CompilerContext ctx, Local valueFrom)
    {
      ...
      {
        using (Local local2 = !returnList || !this.AppendToCollection ? (Local) null : new Local(ctx, this.ExpectedType))
        {
    ...
    {
        moveNext = default(MethodInfo);
        current = default(MethodInfo);
