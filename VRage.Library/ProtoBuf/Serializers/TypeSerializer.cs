...
namespace ProtoBuf.Serializers
{
    internal sealed class TypeSerializer : IProtoTypeSerializer, IProtoSerializer
    {
            ...
            Type constructType, MethodInfo factory)
        {
            ...
            for (int index = 1; index < fieldNumbers.Length; ++index)
            {
                if (fieldNumbers[index] == fieldNumbers[index - 1])
                    throw new InvalidOperationException("Duplicate field-number detected; " +
                                                        fieldNumbers[index].ToString() + " on: " + forType.FullName);
            ...
            this.hasConstructor = !constructType.IsAbstract &&
                                  Helpers.GetConstructor(constructType, Helpers.EmptyTypes, true) !=
        ...
        public bool HasCallbacks(TypeModel.CallbackType callbackType)
        {
            ...
            {
                if (this.serializers[index].ExpectedType != this.forType &&
                    ((IProtoTypeSerializer) this.serializers[index]).HasCallbacks(callbackType))
                    return true;
            }
            return false;
        }
        ...
        private IProtoSerializer GetMoreSpecificSerializer(object value)
        {
            ...
            for (int index = 0; index < this.serializers.Length; ++index)
            {
                ...
                if (protoSerializer.ExpectedType != this.forType &&
                    Helpers.IsAssignableFrom(protoSerializer.ExpectedType, type))
            ...
            while ((num3 = source.ReadFieldHeader()) > 0)
            {
                ...
                for (int index = num2; index < this.fieldNumbers.Length; ++index)
                {
                    if (this.fieldNumbers[index] == num3)
                    {
                        ...
                        else if (protoSerializer.ExpectedType != this.forType &&
                                 ((IProtoTypeSerializer) protoSerializer).CanCreateInstance() &&
                                 protoSerializer.ExpectedType.IsSubclassOf(value.GetType()))
                            value = ProtoReader.Merge(source, value,
                                ((IProtoTypeSerializer) protoSerializer).CreateInstance(source));
        ...
        void IProtoSerializer.EmitWrite(CompilerContext ctx, Local valueFrom)
        {
            ...
            using (Local localWithValue = ctx.GetLocalWithValue(expectedType, valueFrom))
            {
                ...
                if (this.CanHaveInheritance)
                {
                    ...
                    ctx.LoadValue(localWithValue);
                    ctx.EmitCall(ctx.MapType(typeof (object)).GetMethod("GetType"));
                    ctx.EmitCall(ctx.MapType(typeof (TypeModel))
                        .GetMethod("ThrowUnexpectedSubtype",
        ...
        void IProtoTypeSerializer.EmitCallback(CompilerContext ctx, Local valueFrom, TypeModel.CallbackType callbackType)
        {
            ...
            if (this.CanHaveInheritance)
            {
                for (int index = 0; index < this.serializers.Length; ++index)
                {
                    IProtoSerializer protoSerializer = this.serializers[index];
                    if (protoSerializer.ExpectedType != this.forType &&
                        ((IProtoTypeSerializer) protoSerializer).HasCallbacks(callbackType))
                        copyValue = true;
                }
            }
            ...
            for (int index = 0; index < this.serializers.Length; ++index)
            {
                ...
                if (protoSerializer.ExpectedType != this.forType &&
                    (protoTypeSerializer = (IProtoTypeSerializer) protoSerializer).HasCallbacks(callbackType))
                {
                    CodeLabel label2 = ctx.DefineLabel();
                    CodeLabel label3 = ctx.DefineLabel();
                    ctx.CopyValue();
        ...
        void IProtoSerializer.EmitRead(CompilerContext ctx, Local valueFrom)
        {
            ...
            using (Local localWithValue = ctx.GetLocalWithValue(expectedType, valueFrom))
            {
                using (Local local = new Local(ctx, ctx.MapType(typeof (int))))
                {
                    ...
                    ctx.MarkLabel(label1);
                    foreach (
                        BasicList.Group group in
                            BasicList.GetContiguousGroups(this.fieldNumbers, (object[]) this.serializers))
                    {
                        CodeLabel label2 = ctx.DefineLabel();
                        ...
                        if (count == 1)
                        {
                            ...
                            this.WriteFieldHandler(ctx, expectedType, localWithValue, codeLabel2, codeLabel1,
                        ...
                        else
                        {
                            ...
                            ctx.Branch(label2, false);
                            for (int index = 0; index < count; ++index)
                                this.WriteFieldHandler(ctx, expectedType, localWithValue, jumpTable[index], codeLabel1,
                                    (IProtoSerializer) group.Items[index]);
                        }
                        ctx.MarkLabel(label2);
                    }
        ...
        private void WriteFieldHandler(CompilerContext ctx, Type expected, Local loc, CodeLabel handler,
            CodeLabel @continue, IProtoSerializer serializer)
        {
        ...
        void IProtoTypeSerializer.EmitCreateInstance(CompilerContext ctx)
        {
            ...
            else
            {
                ...
                ctx.EmitCall(ctx.MapType(typeof (TypeModel))
                    .GetMethod("ThrowCannotCreateInstance", BindingFlags.Static | BindingFlags.Public));
                ctx.LoadNullRef();
                flag = false;
            }
            if (flag)
            {
                ...
                ctx.EmitCall(ctx.MapType(typeof (ProtoReader))
                    .GetMethod("NoteObject", BindingFlags.Static | BindingFlags.Public));
            }
            if (this.baseCtorCallbacks == null)
                return;
            for (int index = 0; index < this.baseCtorCallbacks.Length; ++index)
