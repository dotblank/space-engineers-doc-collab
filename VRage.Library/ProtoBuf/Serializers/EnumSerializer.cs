          ...
          using (Local local2 = new Local(ctx, ctx.MapType(typeof (int))))
          {
            ...
            foreach (BasicList.Group group in BasicList.GetContiguousGroups(keys, values))
            {
              ...
              if (count == 1)
              {
                ...
                EnumSerializer.WriteEnumValue(ctx, typeCode, codeLabel2, codeLabel1, group.Items[0], local1);
              }
              else
              {
                ctx.LoadValue(local2);
                ...
                ctx.Branch(label, false);
                for (int index = 0; index < count; ++index)
                  EnumSerializer.WriteEnumValue(ctx, typeCode, jumpTable[index], codeLabel1, group.Items[index], local1);
              }
              ctx.MarkLabel(label);
            }
            ctx.LoadReaderWriter();
