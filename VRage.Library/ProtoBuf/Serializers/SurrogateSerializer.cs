...
namespace ProtoBuf.Serializers
{
    internal sealed class SurrogateSerializer : IProtoTypeSerializer, IProtoSerializer
    {
        ...
        public MethodInfo GetConversion(bool toTail)
        {
            ...
            MethodInfo op;
            if (SurrogateSerializer.HasCast(this.declaredType, from, to, out op) ||
                SurrogateSerializer.HasCast(this.forType, from, to, out op))
                return op;
            else
                throw new InvalidOperationException("No suitable conversion operator found for surrogate: " +
                                                    this.forType.FullName + " / " + this.declaredType.FullName);
        }
