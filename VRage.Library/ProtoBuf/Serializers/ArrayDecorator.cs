...
namespace ProtoBuf.Serializers
{
  internal sealed class ArrayDecorator : ProtoDecoratorBase
  {
    ...
    protected override void EmitRead(CompilerContext ctx, Local valueFrom)
    {
      ...
      using (Local local1 = this.AppendToCollection ? ctx.GetLocalWithValue(this.ExpectedType, valueFrom) : (Local) null)
      {
        using (Local local2 = new Local(ctx, this.ExpectedType))
        {
          using (Local local3 = new Local(ctx, type))
          {
            ...
            using (Local local4 = this.AppendToCollection ? new Local(ctx, ctx.MapType(typeof (int))) : (Local) null)
            {
