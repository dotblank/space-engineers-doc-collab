...
namespace ProtoBuf.Compiler
{
  internal class CompilerContext
  {
    ...
    internal CompilerContext(ILGenerator il, bool isStatic, bool isWriter, RuntimeTypeModel.SerializerPair[] methodPairs, TypeModel model, CompilerContext.ILVersion metadataVersion, string assemblyName)
    {
      if (il == null)
    ...
    public void EmitCall(MethodInfo method)
    {
      ...
      this.il.EmitCall(method.IsStatic || method.DeclaringType.IsValueType ? OpCodes.Call : OpCodes.Callvirt, method, (Type[]) null);
    }
    ...
    private bool InternalsVisible(Assembly assembly)
    {
      ...
      if (attributeType == (Type) null)
        return false;
      foreach (InternalsVisibleToAttribute visibleToAttribute in assembly.GetCustomAttributes(attributeType, false))
      {
    ...
    internal void CheckAccessibility(MemberInfo member)
    {
      ...
      switch (member.MemberType)
      {
        ...
        case MemberTypes.NestedType:
          ...
          do
          {
            flag = type.IsNestedPublic || type.IsPublic || (type.DeclaringType == (Type) null || type.IsNestedAssembly || type.IsNestedFamORAssem) && this.InternalsVisible(type.Assembly);
          }
    ...
    internal void LoadValue(Guid value)
    {
      ...
      else
      {
        ...
        this.LoadValue((int) numArray[0] | (int) numArray[1] << 8 | (int) numArray[2] << 16 | (int) numArray[3] << 24);
