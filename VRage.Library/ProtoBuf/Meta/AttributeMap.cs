    ...
    internal abstract class AttributeMap
    {
        ...
        public static AttributeMap[] Create(TypeModel model, Type type, bool inherit)
        {
            ...
            for (int index = 0; index < customAttributes.Length; ++index)
                attributeMapArray[index] =
                    (AttributeMap) new AttributeMap.ReflectionAttributeMap((Attribute) customAttributes[index]);
        ...
        private class ReflectionAttributeMap : AttributeMap
        {
            ...
            {
                get { return (object) this.attribute; }
            }

            public override Type AttributeType
            {
                get { return this.attribute.GetType(); }
            }
            ...
            public override bool TryGet(string key, bool publicOnly, out object value)
            {
                foreach (
                    MemberInfo memberInfo in
                        Helpers.GetInstanceFieldsAndProperties(this.attribute.GetType(), publicOnly))
                {
