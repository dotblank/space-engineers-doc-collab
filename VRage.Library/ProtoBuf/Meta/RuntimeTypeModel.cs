...
namespace ProtoBuf.Meta
{
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        {
            get { return this.GetOption((byte) 1); }
        ...
        {
            get { return this.GetOption((byte) 32); }
            set
            {
        ...
        {
            get { return this.GetOption((byte) 64); }
            set { this.SetOption((byte) 64, value); }
        }

        public static RuntimeTypeModel Default
        {
            get { return RuntimeTypeModel.Singleton.Value; }
        }

        public MetaType this[Type type]
        {
            get { return (MetaType) this.types[this.FindOrAddAuto(type, true, false, false)]; }
        }

        public bool AutoCompile
        {
            get { return this.GetOption((byte) 16); }
            set { this.SetOption((byte) 16, value); }
        }

        public bool AutoAddMissingTypes
        {
            get { return this.GetOption((byte) 8); }
            set
            {
                if (!value && this.GetOption((byte) 2))
        ...
        {
            get { return this.metadataTimeoutMilliseconds; }
            set
            {
                if (value <= 0)
        ...
        public override string GetSchema(Type type)
        {
            ...
            if (flag)
            {
                ...
                MetaType.NewLine(builder2, 1)
                    .Append("optional ")
                    .Append(this.GetSchemaTypeName(type, DataFormat.Default, false, false, ref requiresBclImport))
                    .Append(" value = 1;");
        ...
        private void CascadeDependents(BasicList list, MetaType metaType)
        {
            if (metaType.IsList)
            {
                ...
                WireType defaultWireType;
                if (
                    ValueMember.TryGetCoreSerializer(this, DataFormat.Default, listItemType, out defaultWireType, false,
                ...
                if (metaType.IsAutoTuple)
                {
                    ...
                    if (MetaType.ResolveTupleConstructor(metaType.Type, out mappedMembers) != (ConstructorInfo) null)
                    {
                        for (int index = 0; index < mappedMembers.Length; ++index)
                        {
                            ...
                            WireType defaultWireType;
                            if (
                                ValueMember.TryGetCoreSerializer(this, DataFormat.Default, type, out defaultWireType,
                                    false, false, false, false) == null)
                            {
                                ...
                                if (orAddAuto >= 0)
                                {
                                    MetaType surrogateOrBaseOrSelf =
                                        ((MetaType) this.types[orAddAuto]).GetSurrogateOrBaseOrSelf(false);
                                    if (!list.Contains((object) surrogateOrBaseOrSelf))
                                    {
                                        list.Add((object) surrogateOrBaseOrSelf);
                                        this.CascadeDependents(list, surrogateOrBaseOrSelf);
        ...
        internal int FindOrAddAuto(Type type, bool demand, bool addWithContractOnly, bool addEvenIfAutoDisabled)
        {
            ...
            if (index < 0)
            {
                ...
                try
                {
                    ...
                    {
                        MetaType.AttributeFamily contractFamily = MetaType.GetContractFamily(this, type,
                            (AttributeMap[]) null);
                        ...
                        if (!this.AutoAddMissingTypes && !addEvenIfAutoDisabled ||
                            !Helpers.IsEnum(type) && addWithContractOnly &&
                            contractFamily == MetaType.AttributeFamily.None)
                        {
                            if (demand)
                ...
                if (metaType != null)
                {
                    if (!applyDefaultBehaviour)
                        throw new ArgumentException(
                            ...
                            type.FullName, "applyDefaultBehaviour");
        ...
        private static ILGenerator Override(TypeBuilder type, string name)
        {
            ...
            MethodBuilder methodBuilder = type.DefineMethod(method.Name,
                method.Attributes & ~MethodAttributes.Abstract | MethodAttributes.Final, method.CallingConvention,
        ...
        public TypeModel Compile(RuntimeTypeModel.CompilerOptions options)
        {
            ...
            ModuleBuilder moduleBuilder = flag1
                ? assemblyBuilder.DefineDynamicModule(name2, outputPath)
                : assemblyBuilder.DefineDynamicModule(name2);
            if (!Helpers.IsNullOrEmpty(options.TargetFrameworkName))
            {
                Type type = (Type) null;
                try
                {
                    type = this.GetType("System.Runtime.Versioning.TargetFrameworkAttribute",
                        this.MapType(typeof (string)).Assembly);
                }
                catch
                {
                }
                    ...
                    if (basicList2.IndexOfReference((object) assembly) < 0)
                    {
                        ...
                        for (int index = 0; index < attributeMapArray.Length; ++index)
                        {
                            if (!(attributeMapArray[index].AttributeType != type1))
                            {
                                ...
                                if (!(str == assemblyName) && !Helpers.IsNullOrEmpty(str) &&
                                    basicList1.IndexOf((BasicList.IPredicate) new RuntimeTypeModel.StringFinder(str)) <
                                    0)
                                {
                                    basicList1.Add((object) str);
                                    CustomAttributeBuilder customBuilder =
                                        new CustomAttributeBuilder(type1.GetConstructor(new Type[1]
                                        {
                                            this.MapType(typeof (string))
                                        }), new object[1]
                                        {
            ...
            RuntimeTypeModel.SerializerPair[] serializerPairArray =
                new RuntimeTypeModel.SerializerPair[this.types.Count];
            foreach (MetaType type3 in this.types)
            {
                MethodBuilder serialize = type2.DefineMethod("Write", MethodAttributes.Private | MethodAttributes.Static,
                    CallingConventions.Standard, this.MapType(typeof (void)), new Type[2]
                    {
                        type3.Type,
                        this.MapType(typeof (ProtoWriter))
                    });
                MethodBuilder deserialize = type2.DefineMethod("Read",
                    MethodAttributes.Private | MethodAttributes.Static, CallingConventions.Standard, type3.Type,
                    new Type[2]
                    {
                        type3.Type,
                        this.MapType(typeof (ProtoReader))
                    });
                RuntimeTypeModel.SerializerPair serializerPair =
                    new RuntimeTypeModel.SerializerPair(this.GetKey(type3.Type, true, false),
                        this.GetKey(type3.Type, true, true), type3, serialize, deserialize, serialize.GetILGenerator(),
            ...
            for (int index = 0; index < serializerPairArray.Length; ++index)
            {
                ...
                CompilerContext ctx1 = new CompilerContext(serializerPair.SerializeBody, true, true, serializerPairArray,
                    (TypeModel) this, ilVersion, assemblyName);
                ...
                ctx1.Return();
                CompilerContext ctx2 = new CompilerContext(serializerPair.DeserializeBody, true, false,
                    serializerPairArray, (TypeModel) this, ilVersion, assemblyName);
            ...
            FieldBuilder fieldBuilder = type2.DefineField("knownTypes", type4,
            ...
            switch (num2)
            {
                case 1:
                    ...
                    if (flag2)
                    {
                        ...
                        int num3 = -1;
                        for (int index = 0;
                            index < serializerPairArray.Length &&
                            serializerPairArray[index].MetaKey != serializerPairArray[index].BaseKey;
                            ++index)
                        {
                        ...
                        for (int index = labels.Length - 1; index >= 0; --index)
                        {
                            if (num4 != serializerPairArray[index].BaseKey)
                            {
                                ...
                                {
                                    if (serializerPairArray[length].BaseKey == num4 &&
                                        serializerPairArray[length].MetaKey == num4)
                                    {
                                        num5 = length;
                                        break;
                                    }
                ...
                case 2:
                    ...
                    il1.EmitCall(OpCodes.Callvirt,
                        type4.GetMethod("TryGetValue", BindingFlags.Instance | BindingFlags.Public), (Type[]) null);
            ...
            CompilerContext compilerContext1 = new CompilerContext(il2, false, true, serializerPairArray,
                (TypeModel) this, ilVersion, assemblyName);
            ...
            CompilerContext compilerContext2 = new CompilerContext(il3, false, false, serializerPairArray,
                (TypeModel) this, ilVersion, assemblyName);
            for (int index = 0; index < labels1.Length; ++index)
            ...
            for (int i = 0; i < labels1.Length; ++i)
            {
                ...
                if (type3.IsValueType)
                {
                    ...
                    il3.EmitCall(OpCodes.Call,
                        (MethodInfo)
                            RuntimeTypeModel.EmitBoxedSerializer(type2, i, type3, serializerPairArray, (TypeModel) this,
                                ilVersion, assemblyName), (Type[]) null);
                    compilerContext2.Return();
                }
                else
                {
                ...
                case 1:
                    ...
                    foreach (RuntimeTypeModel.SerializerPair serializerPair in serializerPairArray)
                    {
                        ...
                        ilGenerator.EmitCall(OpCodes.Call,
                            compilerContext2.MapType(typeof (Type)).GetMethod("GetTypeFromHandle"), (Type[]) null);
                ...
                case 2:
                    ...
                    foreach (RuntimeTypeModel.SerializerPair serializerPair in serializerPairArray)
                    {
                        ...
                        if (num4 != serializerPair.MetaKey)
                        {
                            ...
                            for (int index = 0; index < serializerPairArray.Length; ++index)
                            {
                                if (serializerPairArray[index].BaseKey == num4 &&
                                    serializerPairArray[index].MetaKey == num4)
                                {
                                    num3 = index;
                                    break;
                                }
        ...
        private static MethodBuilder EmitBoxedSerializer(TypeBuilder type, int i, Type valueType,
            RuntimeTypeModel.SerializerPair[] methodPairs, TypeModel model, CompilerContext.ILVersion ilVersion,
            string assemblyName)
        {
            MethodInfo method = (MethodInfo) methodPairs[i].Deserialize;
            MethodBuilder methodBuilder = type.DefineMethod("_" + (object) i, MethodAttributes.Static,
                CallingConventions.Standard, model.MapType(typeof (object)), new Type[2]
                {
                    model.MapType(typeof (object)),
                    model.MapType(typeof (ProtoReader))
                });
            CompilerContext ctx = new CompilerContext(methodBuilder.GetILGenerator(), true, false, methodPairs, model,
                ilVersion, assemblyName);
        ...
        internal void ResolveListTypes(Type type, ref Type itemType, ref Type defaultType)
        {
            ...
            if (defaultType == (Type) null && type.IsInterface)
            {
                ...
                if (type.IsGenericType && type.GetGenericTypeDefinition() == this.MapType(typeof (IDictionary<,>)) &&
                    itemType ==
                    this.MapType(typeof (KeyValuePair<,>))
            ...
            ref bool requiresBclImport)
        {
            ...
            if (effectiveType == this.MapType(typeof (byte[])))
                return "bytes";
            WireType defaultWireType;
            IProtoSerializer coreSerializer = ValueMember.TryGetCoreSerializer(this, dataFormat, effectiveType,
        ...
        internal void VerifyFactory(MethodInfo factory, Type type)
        {
            ...
            if (type != (Type) null && factory.ReturnType != type && factory.ReturnType != this.MapType(typeof (object)))
                throw new ArgumentException(
                    "The factory-method must return object" + (type == (Type) null ? "" : " or " + type.FullName),
                    "factory");
        ...
        internal class SerializerPair : IComparable
        {
            ...
            public SerializerPair(int metaKey, int baseKey, MetaType type, MethodBuilder serialize,
                MethodBuilder deserialize, ILGenerator serializeBody, ILGenerator deserializeBody)
            {
        ...
        public sealed class CompilerOptions
        {
            ...
            {
                get { return this.targetFrameworkName; }
                set { this.targetFrameworkName = value; }
            }

            public string TargetFrameworkDisplayName
            {
                get { return this.targetFrameworkDisplayName; }
            ...
            {
                get { return this.imageRuntimeVersion; }
