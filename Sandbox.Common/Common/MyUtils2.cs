...
namespace Sandbox.Common
{
  public static class MyUtils2
  {
    ...
    {
      return (double) (0.0f + MyUtils2.GetAngleBetweenVectorsForSphereCollision(triangle.Vertex0 - point, triangle.Vertex1 - point) + MyUtils2.GetAngleBetweenVectorsForSphereCollision(triangle.Vertex1 - point, triangle.Vertex2 - point) + MyUtils2.GetAngleBetweenVectorsForSphereCollision(triangle.Vertex2 - point, triangle.Vertex0 - point)) >= 6.22035415919481;
    }
    ...
    {
      Vector3 closestPointOnLine1 = MyUtils2.GetClosestPointOnLine(ref triangle.Vertex0, ref triangle.Vertex1, ref sphereCenter);
      if ((double) Vector3.Distance(closestPointOnLine1, sphereCenter) < (double) sphereRadius)
        return new Vector3?(closestPointOnLine1);
      Vector3 closestPointOnLine2 = MyUtils2.GetClosestPointOnLine(ref triangle.Vertex1, ref triangle.Vertex2, ref sphereCenter);
      if ((double) Vector3.Distance(closestPointOnLine2, sphereCenter) < (double) sphereRadius)
        return new Vector3?(closestPointOnLine2);
      Vector3 closestPointOnLine3 = MyUtils2.GetClosestPointOnLine(ref triangle.Vertex2, ref triangle.Vertex0, ref sphereCenter);
    ...
    {
      if (MyUtils2.IsValid(matrix.Up) && MyUtils2.IsValid(matrix.Left) && (MyUtils2.IsValid(matrix.Forward) && MyUtils2.IsValid(matrix.Translation)))
