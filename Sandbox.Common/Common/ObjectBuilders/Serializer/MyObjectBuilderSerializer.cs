...
namespace Sandbox.Common.ObjectBuilders.Serializer
{
    ...
    public class MyObjectBuilderSerializer
    {
        ...
        private static readonly Dictionary<Type, XmlSerializer> m_serializersByType =
            new Dictionary<Type, XmlSerializer>();

        private static readonly Dictionary<string, XmlSerializer> m_serializersBySerializedName =
        ...
        private static readonly MyObjectFactory<MyObjectBuilderDefinitionAttribute, MyObjectBuilder_Base>
            m_objectFactory;
        ...
        static MyObjectBuilderSerializer()
        {
            ...
            MyObjectBuilderSerializer.m_objectFactory =
                new MyObjectFactory<MyObjectBuilderDefinitionAttribute, MyObjectBuilder_Base>();
        ...
        {
            foreach (
                MyObjectBuilderDefinitionAttribute definitionAttribute in
                    MyObjectBuilderSerializer.m_objectFactory.Attributes)
            {
                ...
                MyRuntimeObjectBuilderId runtimeObjectBuilderId =
                    (MyRuntimeObjectBuilderId) (MyObjectBuilderType) definitionAttribute.ProducedType;
                MyObjectBuilderSerializer.Serializer.Add(definitionAttribute.ProducedType.BaseType, true)
                    .AddSubType((int) runtimeObjectBuilderId.Value*1000, definitionAttribute.ProducedType);
            }
        ...
        private static void SerializeXMLInternal(Stream writeTo, MyObjectBuilder_Base objectBuilder,
            Type serializeAsType = null)
        {
            MyObjectBuilderSerializer.m_serializersByType[serializeAsType ?? objectBuilder.GetType()].Serialize(
                writeTo, (object) objectBuilder);
        }

        private static void SerializeGZippedXMLInternal(Stream writeTo, MyObjectBuilder_Base objectBuilder,
            Type serializeAsType = null)
        {
            using (GZipStream gzipStream = new GZipStream(writeTo, CompressionMode.Compress, true))
            {
                using (BufferedStream bufferedStream = new BufferedStream((Stream) gzipStream, 32768))
                    MyObjectBuilderSerializer.SerializeXMLInternal((Stream) bufferedStream, objectBuilder,
                        serializeAsType);
            }
        }

        public static bool SerializeXML(Stream writeTo, MyObjectBuilder_Base objectBuilder,
        ...
        public static bool SerializeXML(string path, bool compress, MyObjectBuilder_Base objectBuilder,
            Type serializeAsType = null)
        {
            ulong sizeInBytes;
            return MyObjectBuilderSerializer.SerializeXML(path, compress, objectBuilder, out sizeInBytes,
                serializeAsType);
        }

        public static bool SerializeXML(string path, bool compress, MyObjectBuilder_Base objectBuilder,
            out ulong sizeInBytes, Type serializeAsType = null)
        {
            try
            {
                using (Stream stream1 = MyFileSystem.OpenWrite(path, FileMode.Create))
                {
                    using (Stream stream2 = compress ? StreamExtensions.WrapGZip(stream1, true) : stream1)
                    {
                        ...
                        MyObjectBuilderSerializer.m_serializersByType[serializeAsType ?? objectBuilder.GetType()]
                            .Serialize(stream2, (object) objectBuilder);
        ...
        public MyObjectBuilder_Base ChangeType(MyObjectBuilderType type, string subtypeName)
        {
            ...
            {
                ((ProtoBuf.Meta.TypeModel) MyObjectBuilderSerializer.Serializer).Serialize((Stream) memoryStream,
                    (object) this);
                memoryStream.Position = 0L;
                MyObjectBuilder_Base newObject = MyObjectBuilderSerializer.CreateNewObject(type, subtypeName);
                return
                    (MyObjectBuilder_Base)
                        ((ProtoBuf.Meta.TypeModel) MyObjectBuilderSerializer.Serializer).Deserialize(
                            (Stream) memoryStream, (object) newObject,
        ...
        public static void MemberwiseAssignment(MyObjectBuilder_Base source, MyObjectBuilder_Base target)
        {
            ...
            foreach (FieldInfo fieldInfo1 in fields1)
            {
                ...
                FieldInfo fieldInfo2 = Enumerable.FirstOrDefault<FieldInfo>((IEnumerable<FieldInfo>) fields2,
                    (Func<FieldInfo, bool>) (fieldInfo =>
                    {
