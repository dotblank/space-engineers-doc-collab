...
namespace Sandbox.Common.ObjectBuilders
{
    public struct MyObjectBuilderType
    {
        ...
        private static Dictionary<string, MyObjectBuilderType> m_typeByName =
            new Dictionary<string, MyObjectBuilderType>(200);
        ...
        private static Dictionary<MyRuntimeObjectBuilderId, MyObjectBuilderType> m_typeById =
            new Dictionary<MyRuntimeObjectBuilderId, MyObjectBuilderType>(200,
                (IEqualityComparer<MyRuntimeObjectBuilderId>) MyRuntimeObjectBuilderId.Comparer);

        private static Dictionary<MyObjectBuilderType, MyRuntimeObjectBuilderId> m_idByType =
            new Dictionary<MyObjectBuilderType, MyRuntimeObjectBuilderId>(200,
                (IEqualityComparer<MyObjectBuilderType>) MyObjectBuilderType.Comparer);
        ...
        public bool IsNull
        {
            get { return this.m_type == (Type) null; }
        }

        static MyObjectBuilderType()
        {
            MyObjectBuilderType.RegisterFromAssembly(Assembly.GetExecutingAssembly(), true);
            MyObjectBuilderType.RegisterLegacyName(
                (MyObjectBuilderType) typeof (MyObjectBuilder_GlobalEventDefinition), "EventDefinition");
            MyObjectBuilderType.RegisterLegacyName((MyObjectBuilderType) typeof (MyObjectBuilder_FactionCollection),
                "Factions");
