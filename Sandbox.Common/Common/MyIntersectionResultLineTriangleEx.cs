  ...
  public struct MyIntersectionResultLineTriangleEx
  {
    ...
    public MyIntersectionResultLineTriangleEx(MyIntersectionResultLineTriangle triangle, IMyEntity entity, ref LineD line)
    {
      ...
      if (this.Entity is IMyVoxelMap)
      {
        ...
        this.IntersectionPointInObjectSpace = (Vector3) (this.IntersectionPointInObjectSpace - ((IMyVoxelMap) this.Entity).PositionLeftBottomCorner);
      }
      else
      {
        MatrixD worldMatrix = this.Entity.WorldMatrix;
        this.NormalInWorldSpace = (Vector3) MyVRageUtils.GetTransformNormalNormalized((Vector3D) this.NormalInObjectSpace, ref worldMatrix);
    ...
    {
      return !a.HasValue && b.HasValue || a.HasValue && b.HasValue && Math.Abs(b.Value.Triangle.Distance - a.Value.Triangle.Distance) <= (double) distanceTolerance;
    }
  }
}
