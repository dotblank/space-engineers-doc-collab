...
namespace Sandbox.Common.Components
{
  public abstract class MyPhysicsComponentBase : MyComponentBase
  {
    ...
    public bool ReportAllContacts
    {
      get
      {
        return (int) this.ContactPointDelay == 0;
      }
      set
      {
        this.ContactPointDelay = value ? (ushort) 0 : ushort.MaxValue;
      }
    }
    ...
    public virtual bool IsStatic
    {
      get
      {
        return (this.Flags & RigidBodyFlag.RBF_STATIC) == RigidBodyFlag.RBF_STATIC;
      }
    }
    ...
    public abstract void CreateCharacterCollision(Vector3 center, float characterWidth, float characterHeight, float crouchHeight, float ladderHeight, float headSize, MatrixD worldTransform, float mass, ushort collisionLayer);
