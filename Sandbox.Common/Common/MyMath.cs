...
namespace Sandbox.Common
{
  public static class MyMath
  {
    ...
    {
      return angle - 6.283185f * (float) Math.Floor(((double) angle + 3.14159297943115 - (double) center) / 6.28318500518799);
    }
    ...
    {
      return new Vector3((float) red / (float) byte.MaxValue, (float) green / (float) byte.MaxValue, (float) blue / (float) byte.MaxValue);
    }

    public static Vector4 VectorFromColor(byte red, byte green, byte blue, byte alpha)
    {
      return new Vector4((float) red / (float) byte.MaxValue, (float) green / (float) byte.MaxValue, (float) blue / (float) byte.MaxValue, (float) alpha / (float) byte.MaxValue);
    }
