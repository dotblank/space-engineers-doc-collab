...
namespace VRageMath
{
  ...
  public struct BoundingBoxD : IEquatable<BoundingBoxD>
  {
    ...
    {
      if (this.Max.X >= box.Min.X && this.Min.X <= box.Max.X && (this.Max.Y >= box.Min.Y && this.Min.Y <= box.Max.Y) && this.Max.Z >= box.Min.Z)
    ...
    public ContainmentType Contains(BoundingSphereD sphere)
    {
      ...
      if (result2 > num * num)
        return ContainmentType.Disjoint;
      return this.Min.X + num <= sphere.Center.X && sphere.Center.X <= this.Max.X - num && (this.Max.X - this.Min.X > num && this.Min.Y + num <= sphere.Center.Y) && (sphere.Center.Y <= this.Max.Y - num && this.Max.Y - this.Min.Y > num && (this.Min.Z + num <= sphere.Center.Z && sphere.Center.Z <= this.Max.Z - num)) && this.Max.X - this.Min.X > num ? ContainmentType.Contains : ContainmentType.Intersects;
    }
    ...
    public void Contains(ref BoundingSphereD sphere, out ContainmentType result)
    {
      ...
      if (result2 > num * num)
        ...
        result = this.Min.X + num > sphere.Center.X || sphere.Center.X > this.Max.X - num || (this.Max.X - this.Min.X <= num || this.Min.Y + num > sphere.Center.Y) || (sphere.Center.Y > this.Max.Y - num || this.Max.Y - this.Min.Y <= num || (this.Min.Z + num > sphere.Center.Z || sphere.Center.Z > this.Max.Z - num)) || this.Max.X - this.Min.X <= num ? ContainmentType.Intersects : ContainmentType.Contains;
    }
