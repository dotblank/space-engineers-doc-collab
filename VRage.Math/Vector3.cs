...
namespace VRageMath
{
  ...
  public struct Vector3 : IEquatable<Vector3>
  {
    ...
    {
      return new Vector3((double) value.X < 0.0 ? -value.X : value.X, (double) value.Y < 0.0 ? -value.Y : value.Y, (double) value.Z < 0.0 ? -value.Z : value.Z);
    }
    ...
    {
      return new Vector3((double) value.X < 0.0 ? -1f : 1f, (double) value.Y < 0.0 ? -1f : 1f, (double) value.Z < 0.0 ? -1f : 1f);
    }
    ...
    {
      float num = a.Dot(b);
      return (double) num * (double) num < 9.99999993922529E-09 * (double) a.LengthSquared() * (double) b.LengthSquared();
    }
    ...
    {
      return new Vector3((double) value.X == 0.0 ? 1f : 0.0f, (double) value.Y == 0.0 ? 1f : 0.0f, (double) value.Z == 0.0 ? 1f : 0.0f);
    }

    public static Vector3 IsZeroVector(Vector3 value, float epsilon)
    {
      return new Vector3((double) Math.Abs(value.X) < (double) epsilon ? 1f : 0.0f, (double) Math.Abs(value.Y) < (double) epsilon ? 1f : 0.0f, (double) Math.Abs(value.Z) < (double) epsilon ? 1f : 0.0f);
    }

    public static Vector3 Step(Vector3 value)
    {
      return new Vector3((double) value.X > 0.0 ? 1f : ((double) value.X < 0.0 ? -1f : 0.0f), (double) value.Y > 0.0 ? 1f : ((double) value.Y < 0.0 ? -1f : 0.0f), (double) value.Z > 0.0 ? 1f : ((double) value.Z < 0.0 ? -1f : 0.0f));
    }
    ...
    {
      return ((int) ((double) this.X * 997.0) * 397 ^ (int) ((double) this.Y * 997.0)) * 397 ^ (int) ((double) this.Z * 997.0);
    }

    public long GetHash()
    {
      return ((((((long) Math.Round((double) Math.Abs(this.X * 1000f)) * 397L ^ (long) Math.Round((double) Math.Abs(this.Y * 1000f))) * 397L ^ (long) Math.Round((double) Math.Abs(this.Z * 1000f))) * 397L ^ (long) (Math.Sign(this.X) + 5)) * 397L ^ (long) (Math.Sign(this.Y) + 7)) * 397L ^ (long) (Math.Sign(this.Z) + 11)) * 397L ^ (long) (2 + 4 + 16 + 256 + 65536 + 1);
    }
    ...
    {
      result = position.X * Base6Directions.GetVector(matrix.Right) + position.Y * Base6Directions.GetVector(matrix.Up) + position.Z * Base6Directions.GetVector(matrix.Backward) + (Vector3) matrix.Translation;
    }

    [SuppressUnmanagedCodeSecurity]
    [DllImport("d3dx9_43.dll", EntryPoint = "D3DXVec3TransformCoord", CallingConvention = CallingConvention.StdCall)]
    private static extern unsafe Vector3* D3DXVec3TransformCoord_([Out] Vector3* pOut, [In] Vector3* pV, [In] Matrix* pM);
    ...
    {
      return (long) ((double) this.X * (double) multiplier) * (long) ((double) this.Y * (double) multiplier) * (long) ((double) this.Z * (double) multiplier);
    }

    public bool IsInsideInclusive(ref Vector3 min, ref Vector3 max)
    {
      if ((double) min.X <= (double) this.X && (double) this.X <= (double) max.X && ((double) min.Y <= (double) this.Y && (double) this.Y <= (double) max.Y) && (double) min.Z <= (double) this.Z)
