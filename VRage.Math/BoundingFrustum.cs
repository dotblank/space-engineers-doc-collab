    ...
    public class BoundingFrustum : IEquatable<BoundingFrustum>
    {
        ...
        public override string ToString()
        {
            return string.Format((IFormatProvider) CultureInfo.CurrentCulture,
                "{{Near:{0} Far:{1} Left:{2} Right:{3} Top:{4} Bottom:{5}}}", (object) this.Near.ToString(),
                (object) this.Far.ToString(), (object) this.Left.ToString(), (object) this.Right.ToString(),
                (object) this.Top.ToString(), (object) this.Bottom.ToString());
        }
        ...
        public void Intersects(ref BoundingBox box, out bool result)
        {
            ...
            do
            {
                ...
                if ((double) result1.X*(double) result4.X + (double) result1.Y*(double) result4.Y +
                    (double) result1.Z*(double) result4.Z > 0.0)
                    return;
        ...
        public ContainmentType Contains(Vector3 point)
        {
            foreach (Plane plane in this.planes)
            {
                if ((double) plane.Normal.X*(double) point.X + (double) plane.Normal.Y*(double) point.Y +
                    (double) plane.Normal.Z*(double) point.Z + (double) plane.D > 9.99999974737875E-06)
                    return ContainmentType.Disjoint;
            }
        ...
        public ContainmentType Contains(BoundingSphere sphere)
        {
            ...
            foreach (Plane plane in this.planes)
            {
                float num3 =
                    (float)
                        ((double) plane.Normal.X*(double) vector3.X + (double) plane.Normal.Y*(double) vector3.Y +
                         (double) plane.Normal.Z*(double) vector3.Z) + plane.D;
                if ((double) num3 > (double) num1)
                    return ContainmentType.Disjoint;
                if ((double) num3 < -(double) num1)
                    ++num2;
