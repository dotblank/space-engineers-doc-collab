...
namespace VRageMath
{
  ...
  public struct Color : IPackedVector<uint>, IPackedVector, IEquatable<Color>
  {
    ...
    {
      this.packedValue = Color.PackHelper((float) color.R / (float) byte.MaxValue, (float) color.G / (float) byte.MaxValue, (float) color.B / (float) byte.MaxValue, a);
    }
    ...
    {
      return (uint) ((int) PackUtils.PackUNorm((float) byte.MaxValue, vectorX) | (int) PackUtils.PackUNorm((float) byte.MaxValue, vectorY) << 8 | (int) PackUtils.PackUNorm((float) byte.MaxValue, vectorZ) << 16 | (int) PackUtils.PackUNorm((float) byte.MaxValue, vectorW) << 24);
    }
