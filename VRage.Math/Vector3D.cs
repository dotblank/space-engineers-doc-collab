...
namespace VRageMath
{
  ...
  public struct Vector3D : IEquatable<Vector3D>
  {
    ...
    {
      return new Vector3D(value.X < 0.0 ? -value.X : value.X, value.Y < 0.0 ? -value.Y : value.Y, value.Z < 0.0 ? -value.Z : value.Z);
    }
    ...
    {
      return new Vector3D(Math.Abs(value.X) < epsilon ? 1.0 : 0.0, Math.Abs(value.Y) < epsilon ? 1.0 : 0.0, Math.Abs(value.Z) < epsilon ? 1.0 : 0.0);
    }

    public static Vector3D Step(Vector3D value)
    {
      return new Vector3D(value.X > 0.0 ? 1.0 : (value.X < 0.0 ? -1.0 : 0.0), value.Y > 0.0 ? 1.0 : (value.Y < 0.0 ? -1.0 : 0.0), value.Z > 0.0 ? 1.0 : (value.Z < 0.0 ? -1.0 : 0.0));
    }
    ...
    public long GetHash()
    {
      return ((((((long) Math.Round(Math.Abs(this.X * 1000.0)) * 397L ^ (long) Math.Round(Math.Abs(this.Y * 1000.0))) * 397L ^ (long) Math.Round(Math.Abs(this.Z * 1000.0))) * 397L ^ (long) (Math.Sign(this.X) + 5)) * 397L ^ (long) (Math.Sign(this.Y) + 7)) * 397L ^ (long) (Math.Sign(this.Z) + 11)) * 397L ^ (long) (2 + 4 + 16 + 256 + 65536 + 1);
    }
    ...
    public static void Transform(Vector3D[] sourceArray, int sourceIndex, ref Matrix matrix, Vector3D[] destinationArray, int destinationIndex, int length)
    {
      for (; length > 0; --length)
      {
        ...
        destinationArray[destinationIndex].X = num1 * (double) matrix.M11 + num2 * (double) matrix.M21 + num3 * (double) matrix.M31 + (double) matrix.M41;
        destinationArray[destinationIndex].Y = num1 * (double) matrix.M12 + num2 * (double) matrix.M22 + num3 * (double) matrix.M32 + (double) matrix.M42;
        destinationArray[destinationIndex].Z = num1 * (double) matrix.M13 + num2 * (double) matrix.M23 + num3 * (double) matrix.M33 + (double) matrix.M43;
        ++sourceIndex;
        ++destinationIndex;
      }
    }
    ...
    public static void TransformNormal(Vector3D[] sourceArray, ref Matrix matrix, Vector3D[] destinationArray)
    {
      for (int index = 0; index < sourceArray.Length; ++index)
      {
        ...
        double num2 = sourceArray[index].Y;
        double num3 = sourceArray[index].Z;
        destinationArray[index].X = num1 * (double) matrix.M11 + num2 * (double) matrix.M21 + num3 * (double) matrix.M31;
        destinationArray[index].Y = num1 * (double) matrix.M12 + num2 * (double) matrix.M22 + num3 * (double) matrix.M32;
        destinationArray[index].Z = num1 * (double) matrix.M13 + num2 * (double) matrix.M23 + num3 * (double) matrix.M33;
      }
    }
    ...
    public static void TransformNormal(Vector3D[] sourceArray, int sourceIndex, ref Matrix matrix, Vector3D[] destinationArray, int destinationIndex, int length)
    {
      for (; length > 0; --length)
      {
        ...
        double num2 = sourceArray[sourceIndex].Y;
        double num3 = sourceArray[sourceIndex].Z;
        destinationArray[destinationIndex].X = num1 * (double) matrix.M11 + num2 * (double) matrix.M21 + num3 * (double) matrix.M31;
        destinationArray[destinationIndex].Y = num1 * (double) matrix.M12 + num2 * (double) matrix.M22 + num3 * (double) matrix.M32;
        destinationArray[destinationIndex].Z = num1 * (double) matrix.M13 + num2 * (double) matrix.M23 + num3 * (double) matrix.M33;
        ++sourceIndex;
        ++destinationIndex;
      }
    }
