...
namespace VRageMath
{
  public class MyDynamicAABBTree
  {
      ...
      else
      {
        ...
        while (!this._nodes[index1].IsLeaf())
        {
          ...
          else
          {
            ...
            index1 = (double) ((float) (this._nodes[index2].Height + 1) * result1.Perimeter()) >= (double) ((float) (this._nodes[index3].Height + 1) * result2.Perimeter()) ? index3 : index2;
          }
        }
        int index4 = index1;
        ...
        for (int iA = this._nodes[leaf].ParentOrNext; iA != -1; iA = this._nodes[iA].ParentOrNext)
        {
          ...
          BoundingBox.CreateMerged(ref this._nodes[index2].Aabb, ref this._nodes[index3].Aabb, out this._nodes[iA].Aabb);
        }
      }
    }
    ...
    public int Balance(int iA)
    {
      ...
      if (num > 1)
      {
        ...
        if (dynamicTreeNode4.Height > dynamicTreeNode5.Height)
        {
          ...
          BoundingBox.CreateMerged(ref dynamicTreeNode2.Aabb, ref dynamicTreeNode5.Aabb, out dynamicTreeNode1.Aabb);
          BoundingBox.CreateMerged(ref dynamicTreeNode1.Aabb, ref dynamicTreeNode4.Aabb, out dynamicTreeNode3.Aabb);
        ...
        else
        {
          ...
          BoundingBox.CreateMerged(ref dynamicTreeNode2.Aabb, ref dynamicTreeNode4.Aabb, out dynamicTreeNode1.Aabb);
          BoundingBox.CreateMerged(ref dynamicTreeNode1.Aabb, ref dynamicTreeNode5.Aabb, out dynamicTreeNode3.Aabb);
      ...
      else
      {
        ...
        if (dynamicTreeNode4.Height > dynamicTreeNode5.Height)
        {
          ...
          dynamicTreeNode5.ParentOrNext = iA;
          BoundingBox.CreateMerged(ref dynamicTreeNode3.Aabb, ref dynamicTreeNode5.Aabb, out dynamicTreeNode1.Aabb);
          BoundingBox.CreateMerged(ref dynamicTreeNode1.Aabb, ref dynamicTreeNode4.Aabb, out dynamicTreeNode2.Aabb);
        ...
        else
        {
          ...
          dynamicTreeNode4.ParentOrNext = iA;
          BoundingBox.CreateMerged(ref dynamicTreeNode3.Aabb, ref dynamicTreeNode4.Aabb, out dynamicTreeNode1.Aabb);
          BoundingBox.CreateMerged(ref dynamicTreeNode1.Aabb, ref dynamicTreeNode5.Aabb, out dynamicTreeNode2.Aabb);
    ...
    public void OverlapAllFrustum<T>(ref BoundingFrustum frustum, List<T> elementsList, List<bool> isInsideList, Vector3 projectionDir, float projectionFactor, float ignoreThr, uint requiredFlags, bool clear = true)
    {
      if (clear)
      {
        elementsList.Clear();
          ...
          if (proxyId1 != -1)
          {
            ...
            if (result == ContainmentType.Contains)
            {
              ...
              while (stack.Count > count)
              {
                ...
                if (dynamicTreeNode2.IsLeaf())
                {
                  if ((double) dynamicTreeNode2.Aabb.ProjectedArea(projectionDir) * (double) projectionFactor > (double) ignoreThr && ((int) this.GetUserFlag(proxyId2) & (int) requiredFlags) == (int) requiredFlags)
                  {
                    elementsList.Add(this.GetUserData<T>(proxyId2));
                    isInsideList.Add(true);
                  }
            ...
            else if (result == ContainmentType.Intersects)
            {
              if (dynamicTreeNode1.IsLeaf())
              {
                if ((double) dynamicTreeNode1.Aabb.ProjectedArea(projectionDir) * (double) projectionFactor > (double) ignoreThr && ((int) this.GetUserFlag(proxyId1) & (int) requiredFlags) == (int) requiredFlags)
                {
                  elementsList.Add(this.GetUserData<T>(proxyId1));
                  isInsideList.Add(false);
                }
