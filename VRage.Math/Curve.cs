    ...
    public class Curve
    {
        ...
        public void ComputeTangent(int keyIndex, CurveTangent tangentInType, CurveTangent tangentOutType)
        {
            ...
            if (tangentInType == CurveTangent.Smooth)
            {
                ...
                curveKey.TangentIn = (double) Math.Abs(num10) >= 1.19209289550781E-07
                    ? num10*Math.Abs(num4 - num3)/num9
                    : 0.0f;
            }
            else
                ...
                return
                    (float)
                        ((double) num3*(2.0*(double) num2 - 3.0*(double) num1 + 1.0) +
                         (double) num4*(-2.0*(double) num2 + 3.0*(double) num1) +
                         (double) num5*((double) num2 - 2.0*(double) num1 + (double) t) +
                         (double) num6*((double) num2 - (double) num1));
            }
        }
    }
}