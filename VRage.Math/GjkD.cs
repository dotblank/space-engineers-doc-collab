        ...
        private void UpdateDeterminant(int xmIdx)
        {
            ...
            while (num2 != 0)
            {
                ...
                for (int index4 = 0; index4 < num3; ++index4)
                {
                    ...
                    this.det[index6][index5] = this.det[index3][index2]*
                                               GjkD.Dot(ref this.edges[index7][index5], ref this.y[index2]) +
                                               this.det[index3][xmIdx]*
                                               GjkD.Dot(ref this.edges[index7][index5], ref this.y[xmIdx]);
                    int index8 = this.edgeLengthSq[index5][index2] < this.edgeLengthSq[xmIdx][index2] ? index5 : xmIdx;
                    this.det[index6][index2] = this.det[num6 | index1][index5]*
                                               GjkD.Dot(ref this.edges[index8][index2], ref this.y[index5]) +
                                               this.det[num6 | index1][xmIdx]*
                                               GjkD.Dot(ref this.edges[index8][index2], ref this.y[xmIdx]);
                    int index9 = this.edgeLengthSq[index2][xmIdx] < this.edgeLengthSq[index5][xmIdx] ? index2 : index5;
                    this.det[index6][xmIdx] = this.det[num4 | num6][index5]*
                                              GjkD.Dot(ref this.edges[index9][xmIdx], ref this.y[index5]) +
                                              this.det[num4 | num6][index2]*
                                              GjkD.Dot(ref this.edges[index9][xmIdx], ref this.y[index2]);
                    num5 >>= 3;
                }
                num2 >>= 3;
                ++num3;
            ...
            if ((this.simplexBits | index1) != 15)
                return;
            int index10 = this.edgeLengthSq[1][0] < this.edgeLengthSq[2][0]
                ? (this.edgeLengthSq[1][0] < this.edgeLengthSq[3][0] ? 1 : 3)
                : (this.edgeLengthSq[2][0] < this.edgeLengthSq[3][0] ? 2 : 3);
            this.det[15][0] = this.det[14][1]*GjkD.Dot(ref this.edges[index10][0], ref this.y[1]) +
                              this.det[14][2]*GjkD.Dot(ref this.edges[index10][0], ref this.y[2]) +
                              this.det[14][3]*GjkD.Dot(ref this.edges[index10][0], ref this.y[3]);
            int index11 = this.edgeLengthSq[0][1] < this.edgeLengthSq[2][1]
                ? (this.edgeLengthSq[0][1] < this.edgeLengthSq[3][1] ? 0 : 3)
                : (this.edgeLengthSq[2][1] < this.edgeLengthSq[3][1] ? 2 : 3);
            this.det[15][1] = this.det[13][0]*GjkD.Dot(ref this.edges[index11][1], ref this.y[0]) +
                              this.det[13][2]*GjkD.Dot(ref this.edges[index11][1], ref this.y[2]) +
                              this.det[13][3]*GjkD.Dot(ref this.edges[index11][1], ref this.y[3]);
            int index12 = this.edgeLengthSq[0][2] < this.edgeLengthSq[1][2]
                ? (this.edgeLengthSq[0][2] < this.edgeLengthSq[3][2] ? 0 : 3)
                : (this.edgeLengthSq[1][2] < this.edgeLengthSq[3][2] ? 1 : 3);
            this.det[15][2] = this.det[11][0]*GjkD.Dot(ref this.edges[index12][2], ref this.y[0]) +
                              this.det[11][1]*GjkD.Dot(ref this.edges[index12][2], ref this.y[1]) +
                              this.det[11][3]*GjkD.Dot(ref this.edges[index12][2], ref this.y[3]);
            int index13 = this.edgeLengthSq[0][3] < this.edgeLengthSq[1][3]
                ? (this.edgeLengthSq[0][3] < this.edgeLengthSq[2][3] ? 0 : 2)
                : (this.edgeLengthSq[1][3] < this.edgeLengthSq[2][3] ? 1 : 2);
            this.det[15][3] = this.det[7][0]*GjkD.Dot(ref this.edges[index13][3], ref this.y[0]) +
                              this.det[7][1]*GjkD.Dot(ref this.edges[index13][3], ref this.y[1]) +
                              this.det[7][2]*GjkD.Dot(ref this.edges[index13][3], ref this.y[2]);
        }
