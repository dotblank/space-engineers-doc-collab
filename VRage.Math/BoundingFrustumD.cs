...
namespace VRageMath
{
    ...
    public class BoundingFrustumD : IEquatable<BoundingFrustumD>
    {
        ...
        public ContainmentType Contains(Vector3D point)
        {
            foreach (PlaneD planeD in this.planes)
            {
                if (planeD.Normal.X*point.X + planeD.Normal.Y*point.Y + planeD.Normal.Z*point.Z + planeD.D >
        ...
        public ContainmentType Contains(BoundingSphereD sphere)
        {
            ...
            foreach (PlaneD planeD in this.planes)
            {
                double num3 = planeD.Normal.X*vector3D.X + planeD.Normal.Y*vector3D.Y + planeD.Normal.Z*vector3D.Z +
                              planeD.D;
                if (num3 > num1)
                    return ContainmentType.Disjoint;
                if (num3 < -num1)
