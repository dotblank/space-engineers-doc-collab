...
namespace VRageMath.Spatial
{
    public class MyClusterTree
    {
        ...
        private Dictionary<ulong, MyClusterTree.MyObjectData> m_objectsData =
            new Dictionary<ulong, MyClusterTree.MyObjectData>();
        ...
        private List<MyLineSegmentOverlapResult<MyClusterTree.MyCluster>> m_lineResultList =
            new List<MyLineSegmentOverlapResult<MyClusterTree.MyCluster>>();
        ...
        public ulong AddObject(BoundingBoxD bbox, Vector3 velocity, MyClusterTree.IMyActivationHandler activationHandler,
            ulong? customId)
        {
            ...
            BoundingBoxD bbox1 = !this.SingleCluster.HasValue
                ? bbox.GetInflated(MyClusterTree.MinimumDistanceFromBorder)
                : bbox;
            this.m_clusterTree.OverlapAllBoundingBox<MyClusterTree.MyCluster>(ref bbox1, this.m_returnedClusters, 0U,
                true);
            ...
            if (this.m_returnedClusters.Count == 1)
            {
                ...
                else if (this.m_returnedClusters[0].AABB.Contains(bbox1) == ContainmentType.Intersects &&
                         activationHandler.IsStaticForCluster)
                {
            ...
            {
                BoundingBoxD boundingBoxD = new BoundingBoxD(bbox.Center - MyClusterTree.IdealClusterSize/2f,
                    bbox.Center + MyClusterTree.IdealClusterSize/2f);
                this.m_clusterTree.OverlapAllBoundingBox<MyClusterTree.MyCluster>(ref boundingBoxD,
                    this.m_returnedClusters, 0U, true);
                if (this.m_returnedClusters.Count == 0)
                {
                    this.m_staticTree.OverlapAllBoundingBox<ulong>(ref boundingBoxD, this.m_objectDataResultList, 0U,
                        true);
        ...
        public void ReorderClusters(BoundingBoxD aabb, ulong objectId)
        {
            ...
            while (!flag1)
            {
                ...
                foreach (MyClusterTree.MyCluster myCluster in this.m_resultList)
                {
                    ...
                    foreach (
                        MyClusterTree.MyObjectData myObjectData in
                            Enumerable
                                .Select<KeyValuePair<ulong, MyClusterTree.MyObjectData>, MyClusterTree.MyObjectData>(
                                    Enumerable.Where<KeyValuePair<ulong, MyClusterTree.MyObjectData>>(
                                        (IEnumerable<KeyValuePair<ulong, MyClusterTree.MyObjectData>>)
                                            this.m_objectsData,
                                        (Func<KeyValuePair<ulong, MyClusterTree.MyObjectData>, bool>)
                                            (x => collidedCluster.Objects.Contains(x.Key))),
                                    (Func<KeyValuePair<ulong, MyClusterTree.MyObjectData>, MyClusterTree.MyObjectData>)
                                        (x => x.Value)))
                ...
                {
                    if (this.m_objectsData[index].Cluster != null &&
                        !this.m_resultList.Contains(this.m_objectsData[index].Cluster))
                    {
            ...
            MyClusterTree.MyClusterDescription clusterDescription1 = new MyClusterTree.MyClusterDescription()
            {
                AABB = bbox,
                DynamicObjects =
                    Enumerable.ToList<MyClusterTree.MyObjectData>(
                        Enumerable.Where<MyClusterTree.MyObjectData>(
                            (IEnumerable<MyClusterTree.MyObjectData>) hashSet1,
                            (Func<MyClusterTree.MyObjectData, bool>) (x => !x.ActivationHandler.IsStaticForCluster))),
                StaticObjects =
                    Enumerable.ToList<MyClusterTree.MyObjectData>(
                        Enumerable.Where<MyClusterTree.MyObjectData>(
                            (IEnumerable<MyClusterTree.MyObjectData>) hashSet1,
            ...
            List<MyClusterTree.MyObjectData> list2 =
                Enumerable.ToList<MyClusterTree.MyObjectData>(
                    Enumerable.Where<MyClusterTree.MyObjectData>(
                        (IEnumerable<MyClusterTree.MyObjectData>) clusterDescription1.StaticObjects,
                        (Func<MyClusterTree.MyObjectData, bool>) (x => x.Cluster != null)));
                ...
                if (clusterDescription2.DynamicObjects.Count != 0)
                {
                    ...
                    for (int index = 0; index < clusterDescription2.DynamicObjects.Count; ++index)
                    {
                        BoundingBoxD inflated =
                            clusterDescription2.DynamicObjects[index].AABB.GetInflated(MyClusterTree.IdealClusterSize/2f);
                    ...
                    {
                        case 0:
                            clusterDescription2.DynamicObjects.Sort(
                                (IComparer<MyClusterTree.MyObjectData>) MyClusterTree.AABBComparerX.Static);
                            break;
                        case 1:
                            clusterDescription2.DynamicObjects.Sort(
                                (IComparer<MyClusterTree.MyObjectData>) MyClusterTree.AABBComparerY.Static);
                            break;
                        case 2:
                            clusterDescription2.DynamicObjects.Sort(
                                (IComparer<MyClusterTree.MyObjectData>) MyClusterTree.AABBComparerZ.Static);
                            break;
                    }
                    bool flag2 = false;
                    if (invalid1.Size().AbsMax() >= (double) MyClusterTree.MaximumForSplit.AbsMax())
                    {
                        for (int index = 1; index < clusterDescription2.DynamicObjects.Count; ++index)
                        {
                            ...
                            if (myObjectData2.AABB.GetInflated(MyClusterTree.IdealClusterSize/2f).Min.GetDim(i) -
                                inflated.Max.GetDim(i) > 0.0)
                            {
                                flag2 = true;
                    ...
                    foreach (
                        MyClusterTree.MyObjectData myObjectData in
                            Enumerable.ToList<MyClusterTree.MyObjectData>(
                                (IEnumerable<MyClusterTree.MyObjectData>) clusterDescription2.DynamicObjects))
                    {
                    ...
                    foreach (
                        MyClusterTree.MyObjectData myObjectData in
                            Enumerable.ToList<MyClusterTree.MyObjectData>(
                                (IEnumerable<MyClusterTree.MyObjectData>) clusterDescription2.StaticObjects))
                    {
                    ...
                    if (clusterDescription2.DynamicObjects.Count > 0)
                    {
                        ...
                        if (clusterDescription4.AABB.Size().AbsMax() >
                            2.0*(double) MyClusterTree.IdealClusterSize.AbsMax())
                            stack.Push(clusterDescription4);
                        else
                            list1.Add(clusterDescription4);
                    }
                    if (clusterDescription3.AABB.Size().AbsMax() > 2.0*(double) MyClusterTree.IdealClusterSize.AbsMax() &&
                        flag2)
