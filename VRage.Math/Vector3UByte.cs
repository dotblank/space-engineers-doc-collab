...
namespace VRageMath
{
    ...
    public struct Vector3UByte
    {
        ...
        [ProtoMember(1)] public byte X;
        [ProtoMember(2)] public byte Y;
        [ProtoMember(3)] public byte Z;
        ...
        {
            return new Vector3UByte((byte) Math.Round((double) vec.X), (byte) Math.Round((double) vec.Y),
                (byte) Math.Round((double) vec.Z));
        }

        public static Vector3UByte Floor(Vector3 vec)
        {
            return new Vector3UByte((byte) Math.Floor((double) vec.X), (byte) Math.Floor((double) vec.Y),
                (byte) Math.Floor((double) vec.Z));
        }
        ...
        public static Vector3 Denormalize(Vector3UByte vec, float range)
        {
            ...
            return (new Vector3((float) vec.X, (float) vec.Y, (float) vec.Z)/(float) byte.MaxValue -
                    new Vector3(0.5f - num))*2f*range;
        }
