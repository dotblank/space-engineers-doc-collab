...
namespace VRageMath.PackedVector
{
    public struct NormalizedByte4 : IPackedVector<uint>, IPackedVector, IEquatable<NormalizedByte4>
    {
        ...
        private static uint PackHelper(float vectorX, float vectorY, float vectorZ, float vectorW)
        {
            return
                (uint)
                    ((int) PackUtils.PackSNorm((uint) byte.MaxValue, vectorX) |
                     (int) PackUtils.PackSNorm((uint) byte.MaxValue, vectorY) << 8 |
                     (int) PackUtils.PackSNorm((uint) byte.MaxValue, vectorZ) << 16 |
                     (int) PackUtils.PackSNorm((uint) byte.MaxValue, vectorW) << 24);
        }
