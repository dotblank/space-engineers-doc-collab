...
namespace VRageMath.PackedVector
{
  public struct Byte4 : IPackedVector<uint>, IPackedVector, IEquatable<Byte4>
  {
    ...
    {
      return (uint) ((int) PackUtils.PackUnsigned((float) byte.MaxValue, vectorX) | (int) PackUtils.PackUnsigned((float) byte.MaxValue, vectorY) << 8 | (int) PackUtils.PackUnsigned((float) byte.MaxValue, vectorZ) << 16 | (int) PackUtils.PackUnsigned((float) byte.MaxValue, vectorW) << 24);
    }
