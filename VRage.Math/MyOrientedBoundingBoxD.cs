...
namespace VRageMath
{
    public struct MyOrientedBoundingBoxD : IEquatable<MyOrientedBoundingBox>
    {
        ...
        {
            return new MyOrientedBoundingBoxD((Vector3D) (Vector3) ((box.Min + box.Max)*0.5),
                (Vector3D) (Vector3) ((box.Max - box.Min)*0.5), Quaternion.Identity);
        }

        public MyOrientedBoundingBoxD Transform(Quaternion rotation, Vector3D translation)
        {
            return new MyOrientedBoundingBoxD(Vector3D.Transform(this.Center, rotation) + translation, this.HalfExtent,
        ...
        {
            return
                new MyOrientedBoundingBoxD(
                    Vector3.Transform((Vector3) (this.Center*(double) scale), rotation) + translation,
                    this.HalfExtent*(double) scale, this.Orientation*rotation);
        }
        ...
        {
            return "{Center:" + this.Center.ToString() + " Extents:" + this.HalfExtent.ToString() + " Orientation:" +
        ...
        public bool Intersects(ref BoundingBox box)
        {
            ...
            return MyOrientedBoundingBoxD.ContainsRelativeBox(ref hA, ref this.HalfExtent, ref fromQuaternion) !=
        ...
        public ContainmentType Contains(ref MyOrientedBoundingBoxD other)
        {
            ...
            return MyOrientedBoundingBoxD.ContainsRelativeBox(ref this.HalfExtent, ref other.HalfExtent,
                ref fromQuaternion);
        }
        ...
        public ContainmentType Contains(ref BoundingSphereD sphere)
        {
            ...
            return num2*num2 + num3*num3 + num4*num4 >= num1*num1
                ? ContainmentType.Disjoint
        ...
        public static ContainmentType Contains(ref BoundingSphere sphere, ref MyOrientedBoundingBox box)
        {
            ...
            return (double) vector3_2.LengthSquared() >= (double) num
                ? ContainmentType.Disjoint
                : ContainmentType.Intersects;
        }
        ...
        public PlaneIntersectionType Intersects(ref PlaneD plane)
        {
            ...
            double num2 = Math.Abs(this.HalfExtent.X*vector3D.X) + Math.Abs(this.HalfExtent.Y*vector3D.Y) +
                          Math.Abs(this.HalfExtent.Z*vector3D.Z);
            if (num1 > num2)
        ...
        public static ContainmentType ContainsRelativeBox(ref Vector3D hA, ref Vector3D hB, ref MatrixD mB)
        {
            ...
            if (vector3D1.X + num1 <= hA.X && vector3D1.Y + num2 <= hA.Y && vector3D1.Z + num3 <= hA.Z)
                return ContainmentType.Contains;
            if (vector3D1.X >= hA.X + Math.Abs(vector3D2.X) + Math.Abs(vector3D3.X) + Math.Abs(vector3D4.X) ||
                vector3D1.Y >= hA.Y + Math.Abs(vector3D2.Y) + Math.Abs(vector3D3.Y) + Math.Abs(vector3D4.Y) ||
                (vector3D1.Z >= hA.Z + Math.Abs(vector3D2.Z) + Math.Abs(vector3D3.Z) + Math.Abs(vector3D4.Z) ||
                 (double) Math.Abs(Vector3.Dot((Vector3) translation, (Vector3) right)) >=
                 Math.Abs(hA.X*right.X) + Math.Abs(hA.Y*right.Y) + Math.Abs(hA.Z*right.Z) + hB.X) ||
                ((double) Math.Abs(Vector3.Dot((Vector3) translation, (Vector3) up)) >=
                 Math.Abs(hA.X*up.X) + Math.Abs(hA.Y*up.Y) + Math.Abs(hA.Z*up.Z) + hB.Y ||
                 (double) Math.Abs(Vector3.Dot((Vector3) translation, (Vector3) backward)) >=
                 Math.Abs(hA.X*backward.X) + Math.Abs(hA.Y*backward.Y) + Math.Abs(hA.Z*backward.Z) + hB.Z))
                return ContainmentType.Disjoint;
            Vector3 vector3_1 = (Vector3) new Vector3D(0.0, -right.Z, right.Y);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_1)) >=
                Math.Abs(hA.Y*(double) vector3_1.Y) + Math.Abs(hA.Z*(double) vector3_1.Z) +
                (double) Math.Abs(Vector3.Dot(vector3_1, (Vector3) vector3D3)) +
                (double) Math.Abs(Vector3.Dot(vector3_1, (Vector3) vector3D4)))
                return ContainmentType.Disjoint;
            Vector3 vector3_2 = (Vector3) new Vector3D(0.0, -up.Z, up.Y);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_2)) >=
                Math.Abs(hA.Y*(double) vector3_2.Y) + Math.Abs(hA.Z*(double) vector3_2.Z) +
                (double) Math.Abs(Vector3.Dot(vector3_2, (Vector3) vector3D4)) +
                (double) Math.Abs(Vector3.Dot(vector3_2, (Vector3) vector3D2)))
                return ContainmentType.Disjoint;
            Vector3 vector3_3 = (Vector3) new Vector3D(0.0, -backward.Z, backward.Y);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_3)) >=
                Math.Abs(hA.Y*(double) vector3_3.Y) + Math.Abs(hA.Z*(double) vector3_3.Z) +
                (double) Math.Abs(Vector3.Dot(vector3_3, (Vector3) vector3D2)) +
                (double) Math.Abs(Vector3.Dot(vector3_3, (Vector3) vector3D3)))
                return ContainmentType.Disjoint;
            Vector3 vector3_4 = (Vector3) new Vector3D(right.Z, 0.0, -right.X);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_4)) >=
                Math.Abs(hA.Z*(double) vector3_4.Z) + Math.Abs(hA.X*(double) vector3_4.X) +
                (double) Math.Abs(Vector3.Dot(vector3_4, (Vector3) vector3D3)) +
                (double) Math.Abs(Vector3.Dot(vector3_4, (Vector3) vector3D4)))
                return ContainmentType.Disjoint;
            Vector3 vector3_5 = (Vector3) new Vector3D(up.Z, 0.0, -up.X);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_5)) >=
                Math.Abs(hA.Z*(double) vector3_5.Z) + Math.Abs(hA.X*(double) vector3_5.X) +
                (double) Math.Abs(Vector3.Dot(vector3_5, (Vector3) vector3D4)) +
                (double) Math.Abs(Vector3.Dot(vector3_5, (Vector3) vector3D2)))
                return ContainmentType.Disjoint;
            Vector3 vector3_6 = (Vector3) new Vector3D(backward.Z, 0.0, -backward.X);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_6)) >=
                Math.Abs(hA.Z*(double) vector3_6.Z) + Math.Abs(hA.X*(double) vector3_6.X) +
                (double) Math.Abs(Vector3.Dot(vector3_6, (Vector3) vector3D2)) +
                (double) Math.Abs(Vector3.Dot(vector3_6, (Vector3) vector3D3)))
                return ContainmentType.Disjoint;
            Vector3 vector3_7 = (Vector3) new Vector3D(-right.Y, right.X, 0.0);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_7)) >=
                Math.Abs(hA.X*(double) vector3_7.X) + Math.Abs(hA.Y*(double) vector3_7.Y) +
                (double) Math.Abs(Vector3.Dot(vector3_7, (Vector3) vector3D3)) +
                (double) Math.Abs(Vector3.Dot(vector3_7, (Vector3) vector3D4)))
                return ContainmentType.Disjoint;
            Vector3 vector3_8 = (Vector3) new Vector3D(-up.Y, up.X, 0.0);
            if ((double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_8)) >=
                Math.Abs(hA.X*(double) vector3_8.X) + Math.Abs(hA.Y*(double) vector3_8.Y) +
                (double) Math.Abs(Vector3.Dot(vector3_8, (Vector3) vector3D4)) +
                (double) Math.Abs(Vector3.Dot(vector3_8, (Vector3) vector3D2)))
                return ContainmentType.Disjoint;
            Vector3 vector3_9 = (Vector3) new Vector3D(-backward.Y, backward.X, 0.0);
            return (double) Math.Abs(Vector3.Dot((Vector3) translation, vector3_9)) >=
                   Math.Abs(hA.X*(double) vector3_9.X) + Math.Abs(hA.Y*(double) vector3_9.Y) +
                   (double) Math.Abs(Vector3.Dot(vector3_9, (Vector3) vector3D2)) +
                   (double) Math.Abs(Vector3.Dot(vector3_9, (Vector3) vector3D3))
                ? ContainmentType.Disjoint
                : ContainmentType.Intersects;
        }
        ...
        {
            MyOrientedBoundingBoxD fromBoundingBox =
                MyOrientedBoundingBoxD.CreateFromBoundingBox(new BoundingBoxD(-boundingBox.Size()/2.0,
                    boundingBox.Size()/2.0));
