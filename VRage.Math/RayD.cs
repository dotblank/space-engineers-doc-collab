    ...
    public struct RayD : IEquatable<RayD>
    {
        ...
        public bool Equals(RayD other)
        {
            if (this.Position.X == other.Position.X && this.Position.Y == other.Position.Y &&
                (this.Position.Z == other.Position.Z && this.Direction.X == other.Direction.X) &&
        ...
        {
            double num1 = (double) plane.Normal.X*this.Direction.X + (double) plane.Normal.Y*this.Direction.Y +
                          (double) plane.Normal.Z*this.Direction.Z;
            if (Math.Abs(num1) < 9.99999974737875E-06)
                return new double?();
            double num2 = (double) plane.Normal.X*this.Position.X + (double) plane.Normal.Y*this.Position.Y +
                          (double) plane.Normal.Z*this.Position.Z;
            double num3 = (-(double) plane.D - num2)/num1;
            if (num3 < 0.0)
            {
