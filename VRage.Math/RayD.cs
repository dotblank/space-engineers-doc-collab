...
namespace VRageMath
{
  ...
  public struct RayD : IEquatable<RayD>
  {
    ...
    {
      double num1 = (double) plane.Normal.X * this.Direction.X + (double) plane.Normal.Y * this.Direction.Y + (double) plane.Normal.Z * this.Direction.Z;
      if (Math.Abs(num1) < 9.99999974737875E-06)
        return new double?();
      double num2 = (double) plane.Normal.X * this.Position.X + (double) plane.Normal.Y * this.Position.Y + (double) plane.Normal.Z * this.Position.Z;
