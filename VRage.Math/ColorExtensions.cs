...
namespace VRageMath
{
    public static class ColorExtensions
    {
        public static Vector3 ColorToHSV(this Color rgb)
        {
            ...
            return new Vector3(color.GetHue()/360f, num1 == 0 ? 0.0f : (float) (1.0 - 1.0*(double) num2/(double) num1),
                (float) num1/(float) byte.MaxValue);
        }

        private static Vector3 Hue(float H)
        {
            return new Vector3(MathHelper.Clamp(Math.Abs((float) ((double) H*6.0 - 3.0)) - 1f, 0.0f, 1f),
                MathHelper.Clamp(2f - Math.Abs((float) ((double) H*6.0 - 2.0)), 0.0f, 1f),
                MathHelper.Clamp(2f - Math.Abs((float) ((double) H*6.0 - 4.0)), 0.0f, 1f));
        }
        ...
        {
            return new Vector3((float) (ushort) packed/360f, (float) ((int) (byte) (packed >> 16) - 100)/100f,
